...
#define maxmodules ( 4000 ) 
#define hashsize ( 8501 ) 
#define bufsize ( 1000 ) 
#define longestname ( 10000 ) 
#define longbufsize ( bufsize + longestname ) 
#define linelength ( 80 ) 
#define maxrefs ( 65535L ) 
#define maxtoks ( 65535L ) 
#define maxtexts ( 10239 ) 
#define maxscraps ( 10000 ) 
#define stacksize ( 2000 ) 
#define maxnewrefs ( 200 ) 
...
struct {
  ...
  sixteenbits dtypefield ;
  sixteenbits dnamefield ;
  sixteenbits dbackfield ;
  sixteenbits dlinkfield ;
} xmem[maxrefs + 1]  ;
xrefnumber xrefptr  ;
short xrefswitch, modxrefswitch  ;
integer deftype, defname, defsubtype, defsubname, constname, packedname  ;
boolean dangerzone  ;
...
integer mm  ;
sixteenbits refloc[maxnewrefs + 1]  ;
sixteenbits reflnk[maxnewrefs + 1]  ;
unsigned char refsafety[maxnewrefs + 1]  ;
integer newrefptr  ;
...
#define noptions ( 3 ) 
  ...
  do {
    ...
    {
      usagehelp ( TWILLHELP , nil ) ;
    } 
    else if ( ( strcmp ( longoptions [optionindex ].name , "version" ) == 0 
    ) ) 
    {
      printversionandexit ( "This is TWILL, Version 4.5" , nil , "D.E. Knuth" 
      , nil ) ;
    } 
  } while ( ! ( getoptreturnval == -1 ) ) ;
  if ( ( optind + 1 > argc ) || ( optind + 3 < argc ) ) 
  {
    fprintf ( stderr , "%s%s\n",  "twill" , ": Need one to three file arguments." ) ;
    usage ( "twill" ) ;
  } 
  ...
  if ( optind + 3 == argc ) 
  texname = extendfilename ( cmdline ( optind + 2 ) , "tex" ) ;
  else texname = basenamechangesuffix ( webname , ".web" , ".tex" ) ;
} 
void 
error ( void ) 
{
...
initialize ( void ) 
{
  ...
  integer h  ;
  ASCIIcode c  ;
  kpsesetprogramname ( argv [0 ], "twill" ) ;
  parsearguments () ;
  history = 0 ;
  xchr [32 ]= ' ' ;
  xchr [33 ]= '!' ;
...
znewxref ( namepointer p ) 
{
  ...
  if ( noxref ) 
  goto lab10 ;
  if ( ( ( ilk [p ]> 3 ) || ( bytestart [p ]+ 1 == bytestart [p + 3 ]) ) 
  && ( xrefswitch == 0 ) ) 
  goto lab10 ;
  ...
  if ( q > 0 ) 
  {
    ...
    goto lab10 ;
      ...
      if ( ( deftype >= 10 ) || dangerzone ) 
      q = xmem [q ].xlinkfield ;
      else {
	  
	xmem [q ].numfield = m ;
	xmem [q ].dtypefield = deftype ;
	xmem [q ].dnamefield = defname ;
	xmem [q ].dbackfield = p ;
	goto lab10 ;
      } 
    } 
  ...
  if ( xrefptr == maxrefs ) 
  {
    ...
    xmem [xrefptr ].dbackfield = 0 ;
  } 
  xmem [xrefptr ].xlinkfield = q ;
  xref [p ]= xrefptr ;
  if ( m > 10240 ) 
  {
    xmem [xrefptr ].dtypefield = deftype ;
    xmem [xrefptr ].dnamefield = defname ;
    xmem [xrefptr ].dbackfield = p ;
    if ( deftype > 10 ) 
    {
      if ( xrefptr == maxrefs ) 
      {
	...
	xrefptr = xrefptr + 1 ;
	xmem [xrefptr ].numfield = 0 ;
	xmem [xrefptr ].dbackfield = 0 ;
      } 
      xmem [xrefptr ].dtypefield = defsubtype ;
      xmem [xrefptr ].dnamefield = defsubname ;
    } 
  } 
  lab10: ;
} 
void 
znewblankxref ( integer p ) 
{
  integer xs  ;
  xs = xrefswitch ;
  xrefswitch = 0 ;
...
zidlookup ( eightbits t ) 
{
  ...
  while ( p != 0 ) {
    ...
    if ( ( bytestart [p + 3 ]- bytestart [p ]== l ) && ( ( ilk [p ]== t 
    ) || ( ( t == 0 ) && ( ilk [p ]> 3 ) ) ) ) 
    {
      i = idfirst ;
      k = bytestart [p ];
  ...
  {
    {
      ...
      {
	putc ('\n',  stdout );
	Fputs ( stdout ,  "! Call to get_line when parsing a module name" ) ;
	error () ;
      } 
    } 
    loc = limit ;
...
scanconst ( void ) 
{
  /* 30 */ register integer Result; integer radix, accum, p  ;
  if ( nextcontrol == 129 ) 
  {
    accum = buffer [idfirst + 1 ];
    nextcontrol = getnext () ;
    goto lab30 ;
  } 
  else if ( nextcontrol == 130 ) 
  {
    p = idlookup ( 0 ) ;
    newblankxref ( p ) ;
    accum = defval [p ];
    ...
    if ( nextcontrol == 13 ) 
    radix = 16 ;
    else if ( nextcontrol == 12 ) 
    radix = 8 ;
    else {
	
      radix = 10 ;
      accum = nextcontrol - 48 ;
    } 
    while ( true ) {
      ...
      if ( radix == 16 ) 
      {
	if ( ( nextcontrol >= 65 ) && ( nextcontrol <= 70 ) ) 
	nextcontrol = nextcontrol - 7 ;
	else if ( nextcontrol > 57 ) 
	goto lab30 ;
      } 
      else if ( nextcontrol >= 48 + radix ) 
      goto lab30 ;
      accum = accum * radix + nextcontrol - 48 ;
    } 
  } 
  lab30: Result = accum ;
  return Result ;
...
scanexp ( void ) 
{
  /* 30 */ register integer Result; integer accum, s  ;
  if ( ( abs ( nextcontrol - 44 ) == 1 ) ) 
  accum = 0 ;
  else accum = scanconst () ;
  while ( true ) {
    ...
    s = 44 - nextcontrol ;
    nextcontrol = getnext () ;
    if ( ! ( ( ( nextcontrol >= 48 ) && ( nextcontrol <= 57 ) ) || ( 
    nextcontrol == 13 ) || ( nextcontrol == 12 ) || ( nextcontrol == 129 ) || 
    ( nextcontrol == 130 ) ) ) 
    goto lab30 ;
    accum = accum + s * scanconst () ;
  } 
  lab30: Result = accum ;
  return Result ;
} 
  ...
  while ( nextcontrol < 143 ) {
	...
	if ( xrefswitch != 0 ) {
	  ...
	  if ( ilk [p ]== 0 ) 
	  {
	    ...
	    if ( ( nextcontrol == 58 ) || ( nextcontrol == 61 ) ) 
	    {
	      eq = ( nextcontrol - 58 ) / ( 3 ) ;
	      nextcontrol = getnext () ;
	      if ( nextcontrol == 130 ) 
		...
		if ( ( ilk [defname ]== 4 ) || ( ilk [defname ]== 18 ) ) 
		{
		  deftype = 4 + eq ;
		  goto lab31 ;
		} 
		if ( ilk [defname ]!= 0 ) 
		goto lab31 ;
		newblankxref ( defname ) ;
		if ( defval [defname ]== 10000000L ) 
		{
		  nextcontrol = getnext () ;
		  if ( nextcontrol == 32 ) 
		  deftype = 13 + eq ;
		  ...
		  if ( abs ( defname ) >= 32768L ) 
		  goto lab32 ;
		  if ( defname < 0 ) 
		  defname = defname + 65536L ;
		  deftype = 11 + eq ;
		} 
	      } 
	      else if ( ( nextcontrol == 129 ) && ( idloc - idfirst > 2 ) ) 
	      {
		deftype = 3 ;
		goto lab31 ;
	      } 
	      else if ( ( ( ( ( nextcontrol >= 48 ) && ( nextcontrol <= 57 ) ) 
	      || ( nextcontrol == 13 ) || ( nextcontrol == 12 ) || ( 
	      nextcontrol == 129 ) || ( nextcontrol == 130 ) ) || ( abs ( 
	      nextcontrol - 44 ) == 1 ) ) ) 
	      {
		defname = scanexp () ;
		if ( nextcontrol != 32 ) {
		...
		deftype = 11 + eq ;
	      ...
	      if ( nextcontrol == 130 ) 
	      {
		defsubname = idlookup ( 0 ) ;
		if ( ilk [defsubname ]!= 0 ) 
		goto lab32 ;
		if ( defval [defsubname ]== 10000000L ) 
		{
		  newblankxref ( defsubname ) ;
		  defsubtype = 13 ;
		  goto lab31 ;
		} 
	      } 
	      if ( ( ( ( ( nextcontrol >= 48 ) && ( nextcontrol <= 57 ) ) || ( 
	      nextcontrol == 13 ) || ( nextcontrol == 12 ) || ( nextcontrol == 
	      129 ) || ( nextcontrol == 130 ) ) || ( abs ( nextcontrol - 44 ) 
	      == 1 ) ) ) 
	      {
		defsubname = scanexp () ;
		defsubtype = 11 ;
		if ( abs ( defsubname ) >= 32768L ) 
		goto lab32 ;
		if ( defsubname < 0 ) 
		defsubname = defsubname + 65536L ;
		goto lab31 ;
	      } 
	      goto lab32 ;
	    } 
    ...
    nextcontrol = getnext () ;
    lab30: if ( ( nextcontrol == 124 ) || ( nextcontrol == 123 ) ) 
    goto lab10 ;
  } 
  lab10: ;
} 
...
breakout ( void ) 
{
  ...
  while ( true ) {
    ...
    if ( ( d == 92 ) && ( outbuf [k - 1 ]!= 92 ) && ( outbuf [k - 1 ]!= 37 
    ) ) 
    {
  ...
  case 2 : 
    {
      tokmem [tokptr ]= 40960L + trans [j ];
      tokptr = tokptr + 1 ;
      tokmem [tokptr ]= 40960L + trans [j + 1 ];
      tokptr = tokptr + 1 ;
    } 
    break ;
  case 3 : 
    {
      ...
      tokmem [tokptr ]= 40960L + trans [j + 1 ];
      tokptr = tokptr + 1 ;
      tokmem [tokptr ]= 40960L + trans [j + 2 ];
      tokptr = tokptr + 1 ;
    } 
    break ;
    default: 
    {
      putc ('\n',  stdout );
      fprintf ( stderr , "%s%s%c",  "! This can't happen (" , "squash" , ')' ) ;
      error () ;
      history = 3 ;
      jumpout () ;
    } 
	...
	if ( ilk [p ]== 0 ) 
	{
	  ...
	  if ( q != 0 ) {
		...
		do {
		    r = q ;
		  q = xmem [q ].dlinkfield ;
		} while ( ! ( ( q == 0 ) || ( xmem [q ].numfield > mm ) ) ) 
		;
		if ( xmem [r ].numfield != mm ) 
		{
		  qq = 0 ;
		  q = reflnk [0 ];
		  while ( refloc [q ]> r ) {
		  ...
		  if ( refloc [q ]== r ) 
		  {
		    ...
		    if ( newrefptr == maxnewrefs ) 
		    {
		      putc ('\n',  stdout );
		      fprintf ( stderr , "%s%s%s",  "! Sorry, " , "new references" ,                       " capacity exceeded" ) ;
		      error () ;
		      history = 3 ;
		      jumpout () ;
		    } 
		    newrefptr = newrefptr + 1 ;
		    reflnk [newrefptr ]= q ;
		    reflnk [qq ]= newrefptr ;
		    refloc [newrefptr ]= r ;
		    if ( xmem [defval [p ]].dlinkfield == 0 ) 
		    refsafety [newrefptr ]= 0 ;
		    else refsafety [newrefptr ]= safety ;
		  } 
		} 
	      } 
	    } 
...
makeoutput ( void ) 
{
  ...
  while ( true ) {
	...
	if ( a == 130 ) {
	  ...
	  if ( bytestart [curname + 3 ]- bytestart [curname ]== 1 ) 
	  {
	    if ( outptr == linelength ) 
	    breakout () ;
	    outptr = outptr + 1 ;
	...
	if ( bytestart [curname + 3 ]- bytestart [curname ]== 1 ) 
	{
	  ...
	  breakout () ;
	  outptr = outptr + 1 ;
	  outbuf [outptr ]= bytemem [curname % 3 ][ bytestart [curname ]]
	  ;
	} 
...
alphaout ( void ) 
{
  ...
  while ( true ) {
    ...
    if ( xx == xrefptr ) 
    goto lab10 ;
    if ( xmem [xx + 1 ].numfield > mm ) 
    goto lab10 ;
    xx = xx + 1 ;
    if ( xmem [xx ].numfield > 10240 ) {
	
      if ( xmem [xx ].dbackfield > 0 ) {
	  
	if ( ilk [xmem [xx ].dbackfield ]== 0 ) 
	{
	  flushbuffer ( outptr , false , false ) ;
	  {
	    if ( outptr == linelength ) 
	  ...
	  p = xmem [xx ].dbackfield ;
	  w = p % 3 ;
	  {register integer for_end; k = bytestart [p ];for_end = 
	  bytestart [p + 3 ]- 1 ; if ( k <= for_end) do 
	    {
	      if ( outptr == linelength ) 
	      breakout () ;
	      outptr = outptr + 1 ;
...
zoutid ( integer p ) 
{
  ...
  if ( ilk [p ]== 0 ) {
    ...
    if ( bytestart [p + 3 ]- bytestart [p ]== 1 ) 
    {
      if ( outptr == linelength ) 
      breakout () ;
      outptr = outptr + 1 ;
  ...
  if ( bytestart [p + 3 ]- bytestart [p ]== 1 ) 
  {
    ...
    breakout () ;
    outptr = outptr + 1 ;
    outbuf [outptr ]= bytemem [p % 3 ][ bytestart [p ]];
  } 
  else outname ( p ) ;
} 
void 
betaout ( void ) 
{
  /* 30 888 31 */ integer k, p, q, w, xx, mmm  ;
  p = reflnk [0 ];
  while ( p != 0 ) {
    ...
    xx = refloc [p ];
    q = xmem [xx ].dbackfield ;
    w = q % 3 ;
    {register integer for_end; k = bytestart [q ];for_end = bytestart [q 
    + 3 ]- 1 ; if ( k <= for_end) do 
      {
	if ( outptr == linelength ) 
	breakout () ;
    ...
    outconst ( xmem [xx ].numfield - 10240 ) ;
    {
      if ( outptr == linelength ) 
      breakout () ;
      outptr = outptr + 1 ;
    ...
    mmm = xmem [xx ].numfield ;
    while ( true ) {
	
      if ( xmem [xx ].dtypefield != 10 ) {
	  
	if ( xmem [xx ].numfield == mmm ) 
	goto lab30 ;
      } 
      if ( xx == xrefptr ) 
      {
	xmem [xx ].dtypefield = 0 ;
	goto lab30 ;
      } 
      if ( xmem [xx + 1 ].numfield > mmm ) 
      {
	xmem [xx ].dtypefield = 0 ;
	goto lab30 ;
      } 
    ...
    if ( xmem [xx + 1 ].dtypefield == 13 ) 
    outid ( xmem [xx + 1 ].dnamefield ) ;
    else outconst ( xmem [xx + 1 ].dnamefield ) ;
    {
      if ( outptr == linelength ) 
      breakout () ;
      outptr = outptr + 1 ;
...
PhaseI ( void ) 
{
  ...
  while ( ! inputhasended ) {
	...
	if ( nextcontrol == 130 ) 
	{
	  lhs = idlookup ( 0 ) ;
	  nextcontrol = getnext () ;
	  if ( ( nextcontrol == 30 ) || ( nextcontrol == 40 ) ) 
	  {
	    deftype = 1 ;
	    newxref ( lhs ) ;
	  } 
	  else if ( nextcontrol == 61 ) 
	  {
	    xrefswitch = 0 ;
	    nextcontrol = getnext () ;
	    defval [lhs ]= scanexp () ;
...
PhaseII ( void ) 
{
  ...
  finishline () ;
  flushbuffer ( 0 , false , false ) ;
  {register integer for_end; lhs = 1 ;for_end = nameptr ; if ( lhs <= 
  for_end) do 
    if ( ilk [lhs ]== 0 ) 
    {
      defval [lhs ]= 0 ;
	...
	if ( xmem [rhs ].numfield > 10240 ) 
	{
	  xmem [rhs ].dlinkfield = defval [lhs ];
	  defval [lhs ]= rhs ;
	} 
	rhs = xmem [rhs ].xlinkfield ;
      } 
    } 
  while ( lhs++ < for_end ) ;} 
  while ( ! inputhasended ) {
      ...
      case 2 : 
	{
	  ...
	  else if ( nextcontrol == 2 ) 
	  {
	    if ( ! phaseone ) 
	    {
	      putc ('\n',  stdout );
	      Fputs ( stdout ,                "! Verbatim string should be in Pascal text only" ) ;
	      error () ;
	    } 
	  } 
	} 
...
void mainbody( void ) {
  ...
  initialize () ;
  Fputs ( stdout ,  "This is TWILL, Version 4.5" ) ;
  fprintf ( stdout , "%s\n",  versionstring ) ;
  idloc = 10 ;
  idfirst = 7 ;
  buffer [7 ]= 97 ;
  ...
  if ( noxref ) 
  {
    ...
    if ( changeexists ) 
    {
      ...
      {
	kmodule = 1 ;
	while ( ! changedmodule [kmodule ]) kmodule = kmodule + 1 ;
	{
	  if ( outptr == linelength ) 
	  breakout () ;
	  outptr = outptr + 1 ;
	...
	outmod ( kmodule ) ;
	do {
	    do { kmodule = kmodule + 1 ;
	  } while ( ! ( changedmodule [kmodule ]) ) ;
	  {
	    if ( outptr == linelength ) 
	    breakout () ;
	    outptr = outptr + 1 ;
	...
	while ( nextname != 0 ) {
	  ...
	  if ( xref [curname ]!= 0 ) 
	  {
	    c = bytemem [curname % 3 ][ bytestart [curname ]];
	    if ( ( c <= 90 ) && ( c >= 65 ) ) 
	    c = c + 32 ;
	...
	do {
	  ...
	  do {
	    ...
	    {
	      outmod ( curval ) ;
	      if ( curval + 1 == xmem [xmem [curxref ].xlinkfield ]
	      .numfield ) 
	      {
		{
		  if ( outptr == linelength ) 
		...
		do {
		    curxref = xmem [curxref ].xlinkfield ;
		  curval = curval + 1 ;
		} while ( ! ( curval + 1 != xmem [xmem [curxref ]
		.xlinkfield ].numfield ) ) ;
		outmod ( curval ) ;
	      } 
	    } 
	...
	do {
	  ...
	  curbank = curname % 3 ;
	  if ( curbyte == bytestart [curname + 3 ]) 
	  c = 0 ;
	  else {
