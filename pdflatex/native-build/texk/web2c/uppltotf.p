...
hashsize=32579;{:3}{157:}maxkanji=3145727;maxkanjicode=3145727;
yokoidnumber=11;tateidnumber=9;{:157}type{17:}byte=0..255;
ASCIIcode=32..127;{:17}{57:}fourbytes=record b0:byte;b1:byte;b2:byte;
b3:byte;end;{:57}{61:}fixword=integer;
{:61}{68:}headerindex=0..maxheaderbytes;indx=0..32767;
{:68}{71:}pointer=0..1032;{:71}var{5:}plfile:text;
...
{:152}{156:}tfmname,plname:constcstring;{:156}{158:}fileformat:0..4;
kanjitype:array[0..maxkanji]of-1..256;kanjitypeindex:0..maxkanji;
nt:integer;glue:array[0..15000]of fixword;ng:integer;
{:158}{148:}procedure parsearguments;const noptions=5;
...
{:151}{154:}longoptions[currentoption].name:='kanji';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:154}{155:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:155};
...
usagehelp(UPPLTOTFHELP,'issue@texjp.org');
end else if(strcmp(longoptions[optionindex].name,'version')=0)then begin
printversionandexit('This is upPLtoTF, Version 3.6-p240427',nil,
'D.E. Knuth',nil);
...
'This is upPLtoTF, Version 3.6-p240427');writeln(stderr,versionstring);
...
{:119}{159:}fileformat:=0;
for kanjitypeindex:=0 to maxkanji do kanjitype[kanjitypeindex]:=-1;
ng:=0;{:159}end;{:2}{27:}procedure showerrorcontext;var k:0..bufsize;
...
{:136}{146:}{166:}procedure checktaggluekern(c:byte);
begin case chartag[c]of 0:;
1:begin if charsonline>0 then writeln(stderr,' ');
...
showerrorcontext;end;end;end;{:166}{175:}function getnextraw:byte;
...
var dig:array[0..5]of byte;i:byte;
begin dig[0]:=(jiscode div 65536)div 16;
dig[1]:=(jiscode div 65536)mod 16;dig[2]:=(jiscode div 4096)mod 16;
dig[3]:=(jiscode div 256)mod 16;dig[4]:=(jiscode div 16)mod 16;
dig[5]:=jiscode mod 16;
for i:=0 to 1 do if(dig[i]<>0)or(dig[0]<>0)then begin if dig[i]<10 then
write(stderr,dig[i])else case dig[i]of 10:write(stderr,'A');
11:write(stderr,'B');12:write(stderr,'C');13:write(stderr,'D');
14:write(stderr,'E');15:write(stderr,'F');end;end;
for i:=2 to 5 do if dig[i]<10 then write(stderr,dig[i])else case dig[i]
...
if(cx>maxkanji)or(not ischarkanji(fromDVI(cx)))or(toDVI(fromDVI(cx))<>cx
...
until ch<>' ';{176:}begin cx:=todig(xord[ch]);loc:=loc+1;
ch:=xord[buffer[loc]];
while((ch>=48)and(ch<=57))or((ch>=65)and(ch<=70))do begin cx:=cx*16+
todig(ch);loc:=loc+1;ch:=xord[buffer[loc]];end;loc:=loc-1;
ch:=xord[buffer[loc]];
if cx>maxkanjicode then begin begin begin if charsonline>0 then writeln(
stderr,' ');write(stderr,'This value shouldn''t exceed jis code');
showerrorcontext;end;repeat getnext until(curchar=40)or(curchar=41);end;
cx:=0;ch:=32;end;end{:176};jiscode:=toDVI(fromJIS(cx));curchar:=ch;
...
stderr,' ');write(stderr,'This value shouldn''t exceed jis code');
showerrorcontext;end;repeat getnext until(curchar=40)or(curchar=41);end;
cx:=0;ch:=32;end;end{:176};jiscode:=toDVI(fromUCS(cx));curchar:=ch;
if not validjiscode(jiscode)then begin if charsonline>0 then writeln(
stderr,' ');write(stderr,'ucs code ',jiscode:1,' is invalid');
showerrorcontext;end;
end else if multistrlen(ustringcast(buffer),loc+4,loc)>1 then begin
curchar:=32;jiscode:=fromBUFF(ustringcast(buffer),loc+4,loc);
loc:=loc+multistrlen(ustringcast(buffer),loc+4,loc)-1;
if(isinternalUPTEX)then begin cx:=fromBUFF(ustringcast(buffer),loc+5,loc
+1);if(UVScombinecode(jiscode,cx)>0)then begin jiscode:=UVScombinecode(
jiscode,cx);loc:=loc+multistrlen(ustringcast(buffer),loc+5,loc+1);end;
end else begin jiscode:=toDVI(jiscode)end;
if not validjiscode(jiscode)then begin if charsonline>0 then writeln(
stderr,' ');write(stderr,'jis code ',jiscode:1,' is invalid');
...
curname[11]:=79;curname[12]:=80;curname[13]:=83;curname[14]:=80;
curname[15]:=65;curname[16]:=67;curname[17]:=73;curname[18]:=78;
curname[19]:=71;curname[20]:=53;entername(33);{:48}{163:}namelength:=10;
...
curname[20]:=75;entername(29);{:163};end;procedure nameenter;
...
entername(85);{:47}{162:}namelength:=4;curname[17]:=84;curname[18]:=89;
...
curname[19]:=85;curname[20]:=69;entername(75);{:162};paramenter;end;
procedure readligkern;var krnptr:0..maxkerns;c:byte;
begin{160:}if fileformat>1 then begin if charsonline>0 then writeln(
stderr,' ');
write(stderr,'This is an illegal command for kanji format files.');
showerrorcontext;end else if fileformat=0 then fileformat:=1{:160};
...
if charwd[g]=0 then begin charwd[g]:=sortin(1,0);
write(stderr,'GLUE character examined by',' ');printoctal(c);
writeln(stderr,' had no CHARACTER spec.');end;end;
end else if ligkern[ligptr].b1<>bchar then begin g:=ligkern[ligptr].b1;
if charwd[g]=0 then begin charwd[g]:=sortin(1,0);
...
if fileformat=1 then begin{125:}if hashptr<hashsize then for hh:=1 to
hashptr do begin tt:=hashlist[hh];
...
printoctal(c);writeln(stderr,'.');end;end{:113};end;
...
showerrorcontext;end else if fileformat=0 then fileformat:=2{:161};
{170:}begin c:=getbyte;
if verbose then{108:}begin if charsonline=8 then begin writeln(stderr,
' ');charsonline:=1;
end else begin if charsonline>0 then write(stderr,' ');
...
if curcode=0 then skiptoendofitem else if(curcode<51)or(curcode>54)then
begin begin if charsonline>0 then writeln(stderr,' ');
...
write(stderr,'You can use this command only for kanji format files.');
showerrorcontext;end else if fileformat=0 then fileformat:=2{:161};
{164:}begin lkstepended:=false;
while level=1 do begin while curchar=32 do getnext;
if curchar=40 then{165:}begin getname;
if curcode=0 then skiptoendofitem else begin case curcode of 70:{167:}
begin while curchar=32 do getnext;
if curchar=66 then begin charremainder[256]:=nl;
repeat getnext until(curchar=40)or(curchar=41);
end else begin begin if(curchar>41)or(curchar<40)then loc:=loc-1;end;
c:=getbyte;checktaggluekern(c);chartag[c]:=1;charremainder[c]:=nl;end;
if minnl<=nl then minnl:=nl+1;lkstepended:=false;end{:167};
71:{99:}if not lkstepended then begin if charsonline>0 then writeln(
stderr,' ');write(stderr,'STOP must follow LIG or KRN');
showerrorcontext;end else begin ligkern[nl-1].b0:=128;
lkstepended:=false;end{:99};
...
showerrorcontext;end else begin ligkern[nl-1].b0:=c;
if minnl<=nl+c then minnl:=nl+c+1;end;lkstepended:=false;end{:100};
73:{168:}begin ligkern[nl].b0:=0;ligkern[nl].b1:=getbyte;
kern[nk]:=getfix;krnptr:=0;
while kern[krnptr]<>kern[nk]do krnptr:=krnptr+1;
if krnptr=nk then begin if nk<maxkerns then nk:=nk+1 else begin begin if
charsonline>0 then writeln(stderr,' ');
write(stderr,'At most ',maxkerns,' different kerns are allowed');
showerrorcontext;end;krnptr:=krnptr-1;end;end;
ligkern[nl].b2:=128+(krnptr div 256);ligkern[nl].b3:=krnptr mod 256;
if nl>=maxligsteps-1 then begin if charsonline>0 then writeln(stderr,' '
);write(stderr,'GLUEKERN table should never exceed ',maxligsteps,
' GLUE/KRN commands');showerrorcontext;end else nl:=nl+1;
lkstepended:=true;end{:168};75:{169:}begin ligkern[nl].b0:=0;
ligkern[nl].b1:=getbyte;glue[3*ng+0]:=getfix;glue[3*ng+1]:=getfix;
...
if krnptr=ng then begin if ng<maxkerns then ng:=ng+1 else begin begin if
charsonline>0 then writeln(stderr,' ');
write(stderr,'At most ',maxkerns,' different glues are allowed');
showerrorcontext;end;krnptr:=krnptr-1;end;end;
ligkern[nl].b2:=krnptr div 256;ligkern[nl].b3:=krnptr mod 256;
if nl>=maxligsteps-1 then begin if charsonline>0 then writeln(stderr,' '
);write(stderr,'GLUEKERN table should never exceed ',maxligsteps,
' GLUE/KRN commands');showerrorcontext;end else nl:=nl+1;
lkstepended:=true;end{:169};
...
end{:165}else if curchar=41 then skiptoendofitem else junkerror;end;
begin loc:=loc-1;level:=level+1;curchar:=41;end;end;{:164};end;
...
memory[2]+memory[3]+memory[4]+nl+lkoffset+nk+ne+np;
if lf<0 then begin writeln(stderr,
'The total number of words in the TFM file too large!');uexit(1);
end{:130};
...
for kanjitypeindex:=0 to maxkanji do begin if kanjitype[kanjitypeindex]>
0 then begin putbyte(((indextojis(kanjitypeindex))mod 65536)div 256,
tfmfile);
putbyte(((indextojis(kanjitypeindex))mod 65536)mod 256,tfmfile);
putbyte((indextojis(kanjitypeindex))div 65536,tfmfile);
putbyte(kanjitype[kanjitypeindex],tfmfile);
