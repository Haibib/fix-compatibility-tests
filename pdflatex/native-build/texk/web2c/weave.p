...
stacksize=2000;{:8}type{11:}ASCIIcode=0..255;
...
debugcycle:integer;debugskipped:integer;}{:258}{269:}noxref:cinttype;
pdfoutput:cinttype;{:269}{271:}webname,chgname,texname:constcstring;
{:271}{264:}procedure parsearguments;const noptions=4;
...
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:266}{267:}longoptions[currentoption].name:=chartostring('p');
...
{:267}{268:}longoptions[currentoption].name:=chartostring('x');
longoptions[currentoption].hasarg:=0;
longoptions[currentoption].flag:=addressof(noxref);
longoptions[currentoption].val:=1;currentoption:=currentoption+1;
{:268}{270:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:270};
...
printversionandexit('This is WEAVE, Version 4.5',nil,'D.E. Knuth',nil);
end;until getoptreturnval=-1;
if(optind+1>argc)or(optind+3<argc)then begin writeln(stderr,'weave',
': Need one to three file arguments.');usage('weave');end;
...
if pdfoutput then write(texfile,'\input pwebma')else write(texfile,
'\input webma');{:124}{126:}outbuf[0]:=92;{:126}{145:}scrapbase:=1;
scrapptr:=0;{maxscrptr:=0;}{:145}{203:}inmodulename:=false;
{maxstackptr:=0;}{:203}{245:}{maxsortptr:=0;}{:245}{248:}collate[0]:=0;
collate[1]:=32;for c:=1 to 31 do collate[c+1]:=c;
...
end else begin 20:if changing then{84:}begin line:=line+1;
if not inputln(changefile)then begin begin if not phaseone then begin
...
write(stdout,'This is WEAVE, Version 4.5');
