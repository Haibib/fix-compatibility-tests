...
loadbasefile ( void ) 
{
  ...
  do {
    ...
    for_end) do 
      undumpwd ( mem [k ]) ;
    while ( k++ < for_end ) ;} 
    if ( ( mem [q ].hhfield .lhfield > lomemmax - q ) || ( mem [q + 1 ]
    .hhfield .v.RH > lomemmax ) || ( ( q >= mem [q + 1 ].hhfield .v.RH ) && 
    ( mem [q + 1 ].hhfield .v.RH != rover ) ) ) 
    goto lab6666 ;
    p = q + mem [q ].hhfield .lhfield ;
    q = mem [q + 1 ].hhfield .v.RH ;
...
void mainbody( void ) {
  ...
  termoffset = 0 ;
  fileoffset = 0 ;
  fprintf ( stdout , "%s%s",  "This is MFLuaJIT, Version 2.71828182" , "-1.0.0-alpha" ) ;
  ...
  outputfilename = 0 ;
  {
    ...
    fixdateandtime () ;
    initrandoms ( systime + sysday * 65536L ) ;
    if ( interaction == 0 ) 
    selector = 0 ;
    else selector = 1 ;
