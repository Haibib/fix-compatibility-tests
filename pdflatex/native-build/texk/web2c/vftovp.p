...
maxstack=100;{:4}{143:}charcodeascii=0;charcodeoctal=1;
charcodedefault=2;{:143}type{5:}byte=0..255;{:5}{22:}indextype=integer;
{:22}{142:}charcodeformattype=charcodeascii..charcodedefault;
{:142}var{7:}vffile:packed file of byte;
...
{:42}{49:}ASCII04,ASCII10,ASCII14:constcstring;
xchr:packed array[0..255]of char;
MBLstring,RIstring,RCEstring:constcstring;
{:49}{51:}dig:array[0..11]of 0..9;{:51}{54:}level:0..5;
{:54}{67:}charsonline:0..8;perfect:boolean;{:67}{69:}i:0..32767;
...
{:85}{87:}activity:array[0..ligsize]of 0..2;ai,acti:0..ligsize;
{:87}{111:}hash:array[0..hashsize]of 0..66048;
...
vflimit:0..vfsize;o:byte;{:123}{139:}verbose:cinttype;
{:139}{144:}charcodeformat:charcodeformattype;
{:144}{147:}tfmname:cstring;vfname,vplname:constcstring;
{:147}{135:}procedure parsearguments;const noptions=4;
var longoptions:array[0..noptions]of getoptstruct;
getoptreturnval:integer;optionindex:cinttype;currentoption:0..noptions;
begin{140:}verbose:=false;{:140}{145:}charcodeformat:=charcodedefault;
{:145};{136:}currentoption:=0;longoptions[currentoption].name:='help';
...
{:136}{137:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:137}{138:}longoptions[currentoption].name:='verbose';
...
{:138}{141:}longoptions[currentoption].name:='charcode-format';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:141}{146:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:146};
...
end;{:135}procedure initialize;var k:integer;
...
RIstring:=' RI ';RCEstring:=' RCE';{:50}{55:}level:=0;
{:55}{68:}charsonline:=0;perfect:=true;{:68}{86:}boundarychar:=256;
bcharlabel:=32767;labelptr:=0;labeltable[0].rr:=0;{:86}end;
...
{:38}{45:}function vfread(k:integer):integer;var b:byte;a:integer;
...
{:56}{57:}procedure outBCPL(k:indextype);var l:0..39;
begin write(vplfile,' ');l:=tfm[k];while l>0 do begin k:=k+1;l:=l-1;
write(vplfile,xchr[tfm[k]]);end;end;
{:57}{58:}procedure outoctal(k,l:indextype);var a:0..1023;b:0..32;
j:0..11;begin write(vplfile,' O ');a:=0;b:=0;j:=0;
while l>0 do{59:}begin l:=l-1;
...
{:60}{61:}procedure outface(k:indextype);var s:0..1;b:0..8;
...
end;end;{:61}{62:}procedure outfix(k:indextype);var a:0..4095;f:integer;
...
end{:63};{64:}begin write(vplfile,'.');f:=10*f+5;delta:=10;
repeat if delta>1048576 then f:=f+524288-(delta div 2);
write(vplfile,f div 1048576:1);f:=10*(f mod 1048576);delta:=delta*10;
until f<=delta;end;{:64};end;
{:62}{74:}procedure checkBCPL(k,l:indextype);var j:indextype;c:byte;
...
{115:}k:=4*(ligkernbase+(i));y:=tfm[k+1];t:=tfm[k+2];cc:=0;zz:=tfm[k+3];
...
{:114}{116:}ifdef('notdef')function ligf(h,x,y:indextype):indextype;
...
{:116}{117:}function ligf(h,x,y:indextype):indextype;
...
ligf:=ligz[h];end;
{:117}{118:}function stringbalance(k,l:integer):boolean;label 45,10;
...
{:118}{120:}procedure outasfix(x:integer);var k:1..3;
...
getbytes:=a;end;{:125}{131:}function vfinput:boolean;
...
{44:}r:=vfptr-(fontstart[fontptr]+14);curname:=xmallocarray(char,r);
for k:=(fontstart[fontptr]+14)to vfptr do begin curname[k-(fontstart[
fontptr]+14)]:=xchr[vf[k]];end;curname[r]:=0;{:44};
...
organize:=vfinput;end;{:131}{132:}procedure dosimplethings;
...
end{:119};{70:}begin fonttype:=0;
if lh>=12 then begin{75:}begin checkBCPL(32,40);
...
outln{:73};{71:}left;write(vplfile,'CHECKSUM');outoctal(24,4);
right{:71};{79:}if(lh>17)and(tfm[92]>127)then begin left;
write(vplfile,'SEVENBITSAFEFLAG TRUE');right;end{:79};end{:70};
{80:}if np>0 then begin left;write(vplfile,'FONTDIMEN');outln;
...
{83:}if i<=7 then case i of 2:write(vplfile,'SPACE');
...
outfix(4*(parambase+i));right;end{:82};right;end;
{81:}if(fonttype=1)and(np<>22)then writeln(stderr,
...
outfix(8);right;right;end{:121};
{84:}if(tfm[4*widthbase]>0)or(tfm[4*widthbase+1]>0)or(tfm[4*widthbase+2]
...
writeln(stderr,'I have set it to zero.');end;{:84};end;
{:132}{133:}function domap(c:byte):boolean;var k:integer;f:0..vfsize;
...
vfptr:=vfptr+1;
if(o<=131)or((o>=133)and(o<=136))then{129:}begin if o>=128 then if o>=
...
end{:129}else case o of{126:}138:;
...
write(vplfile,')');outln;end;
{:127}{128:}171,172,173,174,175,176,177,178,179,180,181,182,183,184,185,
...
{:130}139,140,243,244,245,246,247,248,249,250,251,252,253,254,255:begin
...
end{:124};domap:=true;end;function docharacters:boolean;label 9999,10;
var c:byte;k:indextype;ai:0..ligsize;begin{100:}sortptr:=0;
...
if(tfm[4*(charbase+c)+1]div 16)>0 then{102:}if(tfm[4*(charbase+c)+1]div
...
if(tfm[4*(charbase+c)+1]mod 16)>0 then{103:}if(tfm[4*(charbase+c)+1]mod
...
if(tfm[4*(charbase+c)+2]div 4)>0 then{104:}if(tfm[4*(charbase+c)+2]div 4
...
end else if tfm[k+2]>=128 then{98:}begin if((tfm[k+1]<bc)or(tfm[k+1]>ec)
...
end{:98}else{99:}begin if((tfm[k+1]<bc)or(tfm[k+1]>ec)or(tfm[4*(charbase
...
if tfm[k]>0 then if level=1 then{97:}begin if tfm[k]>=128 then write(
...
end{:105};2:{106:}begin r:=tfm[4*(charbase+c)+3];
...
outchar(tfm[4*(charbase+c)+3]);right;end;end;end{:106};
3:{107:}if tfm[4*(charbase+c)+3]>=ne then begin begin perfect:=false;
...
{108:}for k:=0 to 3 do if(k=3)or(tfm[4*(extenbase+tfm[4*(charbase+c)+3])
...
{88:}if nl>0 then begin for ai:=0 to nl-1 do activity[ai]:=0;
{91:}if tfm[4*(ligkernbase+(0))]=255 then begin left;
...
end;activity[nl-1]:=1;end{:91};end;
{89:}for c:=bc to ec do if(tfm[4*(charbase+c)+2]mod 4)=1 then begin r:=
tfm[4*(charbase+c)+3];
...
tfm[4*(charbase+c)+2]:=4*(tfm[4*(charbase+c)+2]div 4)+0;
end else{90:}begin sortptr:=labelptr;
...
end{:90};end;labeltable[labelptr+1].rr:=ligsize;{:89};
if nl>0 then begin left;write(vplfile,'LIGTABLE');outln;
{92:}for ai:=0 to nl-1 do if activity[ai]=2 then begin r:=tfm[4*(
...
tfm[4*(ligkernbase+(ai))]:=128;end else activity[r]:=2;end;end{:92};
{93:}sortptr:=1;
for acti:=0 to nl-1 do if activity[acti]<>1 then begin i:=acti;
{95:}if activity[i]=0 then begin if level=1 then begin left;
write(vplfile,'COMMENT THIS PART OF THE PROGRAM IS NEVER USED!');outln;
end end else if level=2 then right{:95};
{94:}while i=labeltable[sortptr].rr do begin left;
write(vplfile,'LABEL');
if labeltable[sortptr].cc=256 then write(vplfile,' BOUNDARYCHAR')else
outchar(labeltable[sortptr].cc);right;sortptr:=sortptr+1;end{:94};
{96:}begin k:=4*(ligkernbase+(i));
if tfm[k]>128 then begin if 256*tfm[k+2]+tfm[k+3]>=nl then begin perfect
:=false;if charsonline>0 then writeln(stderr,' ');charsonline:=0;
writeln(stderr,'Bad TFM file: ',
'Ligature unconditional stop command address is too big.');end;
...
{109:}if ne>0 then for c:=0 to ne-1 do for d:=0 to 3 do begin k:=4*(
extenbase+c)+d;
