{2:}{4:}{$C-,A+,D-}{[$C+,D+]}
{:4}program WEAVE(webfile,changefile,texfile);const{8:}maxbytes=65535;
maxnames=10239;maxmodules=4000;hashsize=8501;bufsize=1000;
longestname=10000;longbufsize=bufsize+longestname;linelength=80;
maxrefs=65535;maxtoks=65535;maxtexts=10239;maxscraps=10000;
stacksize=2000;maxnewrefs=200;{:8}type{11:}ASCIIcode=0..255;
...
{:29}{37:}bytemem:packed array[0..2,0..maxbytes]of ASCIIcode;
...
defval:array[0..maxnames]of integer;{:37}{39:}nameptr:namepointer;
byteptr:array[0..2]of 0..maxbytes;{:39}{45:}modulecount:0..maxmodules;
...
xlinkfield:sixteenbits;dtypefield:sixteenbits;dnamefield:sixteenbits;
dbackfield:sixteenbits;dlinkfield:sixteenbits;end;xrefptr:xrefnumber;
xrefswitch,modxrefswitch:0..10240;
deftype,defname,defsubtype,defsubname,constname,packedname:integer;
dangerzone:boolean;
...
{:177}{202:}inmodulename:boolean;curstate:outputstate;
...
{:242}{244:}curdepth:eightbits;curbyte:0..maxbytes;curbank:0..2;
...
{:268}{270:}webname,chgname,texname:constcstring;{:270}{279:}mm:integer;
refloc:array[0..maxnewrefs]of sixteenbits;
reflnk:array[0..maxnewrefs]of sixteenbits;
refsafety:array[0..maxnewrefs]of 0..1;newrefptr:0..maxnewrefs;
safety:0..1;xx:xrefnumber;{:279}{264:}procedure parsearguments;
...
end else if getoptreturnval=63 then begin usage('twill');
...
printversionandexit('This is TWILL, Version 4.5',nil,'D.E. Knuth',nil);
end;until getoptreturnval=-1;
if(optind+1>argc)or(optind+3<argc)then begin writeln(stderr,'twill',
': Need one to three file arguments.');usage('twill');end;
...
if optind+3=argc then texname:=extendfilename(cmdline(optind+2),'tex')
else texname:=basenamechangesuffix(webname,'.web','.tex');end;
...
for curbank:=1 to 2 do write(stdout,'+',byteptr[curbank]:1);
...
end;{:33}procedure initialize;var{16:}i:0..255;{:16}{40:}wi:0..2;
{:40}{56:}h:0..hashsize;{:56}{247:}c:ASCIIcode;
{:247}begin kpsesetprogramname(argv[0],'twill');parsearguments;
...
{:26}{41:}for wi:=0 to 2 do begin bytestart[wi]:=0;byteptr[wi]:=0;end;
bytestart[3]:=0;nameptr:=1;{:41}{43:}ilk[0]:=0;{:43}{49:}xrefptr:=0;
xrefswitch:=0;modxrefswitch:=0;xmem[0].numfield:=0;dangerzone:=false;
...
{:102}{124:}outptr:=1;outline:=1;outbuf[1]:=98;
write(texfile,'\input twimac-we');{:124}{126:}outbuf[0]:=92;
{:126}{145:}scrapbase:=1;scrapptr:=0;{maxscrptr:=0;}
{:145}{203:}inmodulename:=false;{maxstackptr:=0;}
{:203}{245:}{maxsortptr:=0;}{:245}{248:}collate[0]:=0;collate[1]:=32;
...
{:28}{44:}procedure printid(p:namepointer);var k:0..maxbytes;w:0..2;
...
if((ilk[p]>3)or(bytestart[p]+1=bytestart[p+3]))and(xrefswitch=0)then
goto 10;m:=modulecount+xrefswitch;xrefswitch:=0;q:=xref[p];
if q>0 then begin n:=xmem[q].numfield;
if(n=m)or(n=m+10240)then goto 10 else if m=n+10240 then if(deftype>=10)
or dangerzone then q:=xmem[q].xlinkfield else begin xmem[q].numfield:=m;
xmem[q].dtypefield:=deftype;xmem[q].dnamefield:=defname;
xmem[q].dbackfield:=p;goto 10;end;end;
...
xmem[xrefptr].numfield:=m;xmem[xrefptr].dbackfield:=0;end;
xmem[xrefptr].xlinkfield:=q;xref[p]:=xrefptr;
if m>10240 then begin xmem[xrefptr].dtypefield:=deftype;
xmem[xrefptr].dnamefield:=defname;xmem[xrefptr].dbackfield:=p;
if deftype>10 then begin if xrefptr=maxrefs then begin writeln(stdout);
write(stderr,'! Sorry, ','cross reference',' capacity exceeded');error;
history:=3;jumpout;end else begin xrefptr:=xrefptr+1;
xmem[xrefptr].numfield:=0;xmem[xrefptr].dbackfield:=0;end;
xmem[xrefptr].dtypefield:=defsubtype;
xmem[xrefptr].dnamefield:=defsubname;end;end;10:end;
procedure newblankxref(p:integer);var xs:integer;begin xs:=xrefswitch;
xrefswitch:=0;newxref(p);xrefswitch:=xs;end;
{:50}{51:}procedure newmodxref(p:namepointer);var q,r:xrefnumber;
begin q:=xref[p];r:=0;
...
{60:}p:=hash[h];
while p<>0 do begin if(bytestart[p+3]-bytestart[p]=l)and((ilk[p]=t)or((t
=0)and(ilk[p]>3)))then{61:}begin i:=idfirst;k:=bytestart[p];w:=p mod 3;
...
if p=nameptr then{62:}begin w:=nameptr mod 3;
if byteptr[w]+l>maxbytes then begin writeln(stdout);
write(stderr,'! Sorry, ','byte memory',' capacity exceeded');error;
history:=3;jumpout;end;if nameptr+3>maxnames then begin writeln(stdout);
...
byteptr[w]:=k;bytestart[nameptr+3]:=k;nameptr:=nameptr+1;ilk[p]:=t;
xref[p]:=0;defval[p]:=10000000;end{:62};idlookup:=p;end;
...
write(stderr,'! Sorry, ','byte memory',' capacity exceeded');error;
history:=3;jumpout;end;if nameptr>maxnames-3 then begin writeln(stdout);
write(stderr,'! Sorry, ','name',' capacity exceeded');error;history:=3;
...
byteptr[w]:=k+l;bytestart[nameptr+3]:=k+l;nameptr:=nameptr+1;{:67};
...
label 20;
begin if inmodulename then begin begin if not phaseone then begin
writeln(stdout);
write(stdout,'! Call to get_line when parsing a module name');error;end;
end;loc:=limit;
end else begin 20:if changing then changedmodule[modulecount]:=true else
{83:}begin line:=line+1;
...
end;loc:=0;buffer[limit]:=32;end;end;
...
{88:}{48,49,50:begin tracing:=c-48;controlcode:=0;
51:begin['*****************************************************']
controlcode:=0;end;end;}
...
{:95}{111:}{271:}function scanconst:integer;label 30;
var radix,accum,p:integer;
begin if nextcontrol=129 then begin accum:=buffer[idfirst+1];
nextcontrol:=getnext;goto 30;
end else if nextcontrol=130 then begin p:=idlookup(0);newblankxref(p);
accum:=defval[p];nextcontrol:=getnext;goto 30;end else begin accum:=0;
if nextcontrol=13 then radix:=16 else if nextcontrol=12 then radix:=8
else begin radix:=10;accum:=nextcontrol-48;end;
while true do begin nextcontrol:=getnext;if nextcontrol<48 then goto 30;
if radix=16 then begin if(nextcontrol>=65)and(nextcontrol<=70)then
nextcontrol:=nextcontrol-7 else if nextcontrol>57 then goto 30;
end else if nextcontrol>=48+radix then goto 30;
accum:=accum*radix+nextcontrol-48;end;end;30:scanconst:=accum;end;
{:271}{272:}function scanexp:integer;label 30;var accum,s:integer;
begin if(abs(nextcontrol-44)=1)then accum:=0 else accum:=scanconst;
while true do begin if not(abs(nextcontrol-44)=1)then goto 30;
s:=44-nextcontrol;nextcontrol:=getnext;
if not(((nextcontrol>=48)and(nextcontrol<=57))or(nextcontrol=13)or(
nextcontrol=12)or(nextcontrol=129)or(nextcontrol=130))then goto 30;
accum:=accum+s*scanconst;end;30:scanexp:=accum;end;
{:272}procedure Pascalxref;label 10,30,31,32;var p:namepointer;eq:0..3;
begin while nextcontrol<143 do begin if(nextcontrol>=130)and(nextcontrol
<=133)then begin p:=idlookup(nextcontrol-130);
if nextcontrol=130 then if xrefswitch<>0 then if ilk[p]=0 then{273:}
begin nextcontrol:=getnext;if nextcontrol=44 then begin deftype:=10;
goto 31;end;
if(nextcontrol=58)or(nextcontrol=61)then{274:}begin eq:=(nextcontrol-58)
div(3);nextcontrol:=getnext;
if nextcontrol=130 then{275:}begin defname:=idlookup(0);
if ilk[defname]=13 then begin nextcontrol:=getnext;eq:=eq+2;
if nextcontrol<>130 then goto 32;defname:=idlookup(0);end;
if(ilk[defname]=4)or(ilk[defname]=18)then begin deftype:=4+eq;goto 31;
end;if ilk[defname]<>0 then goto 31;newblankxref(defname);
if defval[defname]=10000000 then begin nextcontrol:=getnext;
if nextcontrol=32 then deftype:=13+eq else begin deftype:=8+eq;goto 31;
end;end else{276:}begin defname:=scanexp;
if nextcontrol<>32 then if eq=1 then begin deftype:=5;
defname:=constname;goto 31;end else goto 32;
if abs(defname)>=32768 then goto 32;
if defname<0 then defname:=defname+65536;deftype:=11+eq;end{:276};
end{:275}else if(nextcontrol=129)and(idloc-idfirst>2)then begin deftype
:=3;goto 31;
end else if((((nextcontrol>=48)and(nextcontrol<=57))or(nextcontrol=13)or
(nextcontrol=12)or(nextcontrol=129)or(nextcontrol=130))or(abs(
nextcontrol-44)=1))then{276:}begin defname:=scanexp;
if nextcontrol<>32 then if eq=1 then begin deftype:=5;
defname:=constname;goto 31;end else goto 32;
if abs(defname)>=32768 then goto 32;
if defname<0 then defname:=defname+65536;deftype:=11+eq;
end{:276}else goto 32;nextcontrol:=getnext;
if nextcontrol=130 then begin defsubname:=idlookup(0);
if ilk[defsubname]<>0 then goto 32;
if defval[defsubname]=10000000 then begin newblankxref(defsubname);
defsubtype:=13;goto 31;end;end;
if((((nextcontrol>=48)and(nextcontrol<=57))or(nextcontrol=13)or(
nextcontrol=12)or(nextcontrol=129)or(nextcontrol=130))or(abs(nextcontrol
-44)=1))then begin defsubname:=scanexp;defsubtype:=11;
if abs(defsubname)>=32768 then goto 32;
if defsubname<0 then defsubname:=defsubname+65536;goto 31;end;goto 32;
end{:274};32:deftype:=0;newxref(p);goto 30;31:end{:273};newxref(p);
dangerzone:=(deftype=10);
if(ilk[p]=17)or(ilk[p]=22)then xrefswitch:=10240;
if ilk[p]=17 then begin defname:=p;nextcontrol:=getnext;
if nextcontrol<>130 then goto 30;if xrefswitch=0 then goto 30;
p:=idlookup(0);deftype:=4;newxref(p);end;end;nextcontrol:=getnext;
30:if(nextcontrol=124)or(nextcontrol=123)then goto 10;end;10:end;
{:111}{112:}procedure outerxref;var bal:eightbits;
...
goto 10;end;
if(d=92)and(outbuf[k-1]<>92)and(outbuf[k-1]<>37)then begin flushbuffer(k
-1,true,true);goto 10;end;k:=k-1;end;10:end;
{:127}{130:}procedure outmod(m:integer);var k:0..5;a:integer;begin k:=0;
a:=m;repeat dig[k]:=a mod 10;a:=a div 10;k:=k+1;until a=0;repeat k:=k-1;
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=dig[k]+48;end;until k=0;end;
{:130}{131:}procedure outname(p:namepointer);var k:0..maxbytes;w:0..2;
begin begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=123;end;w:=p mod 3;
for k:=bytestart[p]to bytestart[p+3]-1 do begin if bytemem[w,k]=95 then
begin if outptr=linelength then breakout;outptr:=outptr+1;
...
d:integer);var i:0..maxscraps;begin cat[j]:=c;
if k>0 then begin trans[j]:=textptr;textptr:=textptr+1;
...
d:integer);begin case k of 1:begin tokmem[tokptr]:=40960+trans[j];
tokptr:=tokptr+1;end;2:begin tokmem[tokptr]:=40960+trans[j];
tokptr:=tokptr+1;tokmem[tokptr]:=40960+trans[j+1];tokptr:=tokptr+1;end;
3:begin tokmem[tokptr]:=40960+trans[j];tokptr:=tokptr+1;
tokmem[tokptr]:=40960+trans[j+1];tokptr:=tokptr+1;
tokmem[tokptr]:=40960+trans[j+2];tokptr:=tokptr+1;end;
others:begin writeln(stdout);
write(stderr,'! This can''t happen (','squash',')');error;history:=3;
jumpout;end end;red(j,k,c,d);end;
...
tokmem[tokptr]:=40960+trans[pp+1];tokptr:=tokptr+1;begin red(pp,2,5,-1);
{prod(6)};goto 31;end;end{:152};
...
begin red(pp,2,11,-2);{prod(17)};goto 31;end;end{:159};
...
tokptr:=tokptr+1;begin red(pp,1,11,-2);{prod(18)};goto 31;end;
...
tokmem[tokptr]:=40960+trans[pp+1];tokptr:=tokptr+1;begin red(pp,2,3,-3);
{prod(19)};goto 31;end;end else if cat[pp+1]=2 then begin sq(pp,2,2,-1);
...
tokptr:=tokptr+1;begin red(pp,2,11,-2);{prod(22)};goto 31;end;
...
begin red(pp,2,11,-2);{prod(23)};goto 31;end;end{:160};
...
begin red(pp,3,2,-1);{prod(26)};goto 31;end;
...
tokptr:=tokptr+1;tokmem[tokptr]:=40960+trans[pp+1];tokptr:=tokptr+1;
begin red(pp,2,2,-1);{prod(27)};goto 31;end;
...
tokptr:=tokptr+1;begin red(pp,4,2,-1);{prod(28)};goto 31;end;
...
tokptr:=tokptr+1;begin red(pp+1,3,2,0);{prod(31)};goto 31;end;end;
...
tokptr:=tokptr+1;begin red(pp+1,2,2,0);{prod(32)};goto 31;end;
end else if cat[pp+2]=19 then begin if cat[pp+3]=3 then begin tokmem[
tokptr]:=40960+trans[pp+1];tokptr:=tokptr+1;tokmem[tokptr]:=133;
tokptr:=tokptr+1;tokmem[tokptr]:=135;tokptr:=tokptr+1;
tokmem[tokptr]:=40960+trans[pp+2];tokptr:=tokptr+1;tokmem[tokptr]:=125;
tokptr:=tokptr+1;begin red(pp+1,3,2,0);{prod(33)};goto 31;end;end;end;
end{:163}else if cat[pp+1]=16 then begin if cat[pp+2]=3 then begin
tokmem[tokptr]:=133;tokptr:=tokptr+1;tokmem[tokptr]:=135;
tokptr:=tokptr+1;tokmem[tokptr]:=40960+trans[pp+1];tokptr:=tokptr+1;
tokmem[tokptr]:=125;tokptr:=tokptr+1;begin red(pp+1,2,2,0);{prod(34)};
goto 31;end;end;end else if cat[pp+1]=1 then begin red(pp+1,0,2,0);
{prod(35)};goto 31;
...
tokptr:=tokptr+1;begin red(pp,3,2,-1);{prod(36)};goto 31;end;
...
tokptr:=tokptr+1;begin red(pp+1,2,2,0);{prod(37)};goto 31;end;end;
end{:162};1:{167:}if cat[pp+1]=6 then begin red(pp,0,11,-2);{prod(43)};
goto 31;end else if cat[pp+1]=14 then begin tokmem[tokptr]:=141;
tokptr:=tokptr+1;tokmem[tokptr]:=139;tokptr:=tokptr+1;
begin sq(pp,2,3,-3);{prod(44)};goto 31;end;
end else if cat[pp+1]=2 then begin sq(pp,2,2,-1);{prod(45)};goto 31;
end else if cat[pp+1]=22 then begin sq(pp,2,22,0);{prod(46)};goto 31;
end else if cat[pp+1]=1 then begin sq(pp,2,1,-2);{prod(47)};goto 31;
end else if cat[pp+1]=10 then begin sq(pp,2,11,-2);{prod(48)};goto 31;
end{:167};others:end;pp:=pp+1;31:end;procedure alphacases;label 31;
...
tokptr:=tokptr+1;begin red(pp,3,13,-2);{prod(2)};goto 31;end;end;
...
tokptr:=tokptr+1;begin red(pp,2,13,-2);{prod(3)};goto 31;end;
end else if cat[pp+1]=1 then begin red(pp+1,0,2,0);{prod(4)};goto 31;
...
tokptr:=tokptr+1;tokmem[tokptr]:=40960+trans[pp+2];tokptr:=tokptr+1;
begin red(pp,3,17,0);{prod(7)};goto 31;end;end;
...
begin red(pp,3,11,-2);{prod(8)};goto 31;end;end;
end else if cat[pp+1]=11 then begin tokmem[tokptr]:=40960+trans[pp];
tokptr:=tokptr+1;tokmem[tokptr]:=141;tokptr:=tokptr+1;
tokmem[tokptr]:=40960+trans[pp+1];tokptr:=tokptr+1;begin red(pp,2,17,0);
{prod(9)};goto 31;end;end{:153};
...
tokptr:=tokptr+1;tokmem[tokptr]:=137;tokptr:=tokptr+1;
tokmem[tokptr]:=141;tokptr:=tokptr+1;begin red(pp,2,11,-2);{prod(11)};
goto 31;end;end{:155};
...
tokptr:=tokptr+1;tokmem[tokptr]:=135;tokptr:=tokptr+1;
begin red(pp,4,13,-2);{prod(12)};goto 31;end;
end else begin tokmem[tokptr]:=40960+trans[pp];tokptr:=tokptr+1;
tokmem[tokptr]:=40960+trans[pp+1];tokptr:=tokptr+1;tokmem[tokptr]:=140;
tokptr:=tokptr+1;tokmem[tokptr]:=40960+trans[pp+2];tokptr:=tokptr+1;
tokmem[tokptr]:=135;tokptr:=tokptr+1;tokmem[tokptr]:=137;
tokptr:=tokptr+1;tokmem[tokptr]:=141;tokptr:=tokptr+1;
begin red(pp,3,11,-2);{prod(13)};goto 31;end;end{:156};
20:{157:}begin red(pp,0,3,-3);{prod(14)};goto 31;end{:157};
15:{158:}if cat[pp+1]=2 then begin if cat[pp+2]=1 then if cat[pp+3]<>1
then begin tokmem[tokptr]:=40960+trans[pp];tokptr:=tokptr+1;
tokmem[tokptr]:=40960+trans[pp+1];tokptr:=tokptr+1;
tokmem[tokptr]:=40960+trans[pp+2];tokptr:=tokptr+1;tokmem[tokptr]:=125;
tokptr:=tokptr+1;begin red(pp,3,2,-1);{prod(15)};goto 31;end;end;
...
begin red(pp,2,2,-1);{prod(16)};goto 31;end;end{:158};
22:{161:}if(cat[pp+1]=10)or(cat[pp+1]=9)then begin tokmem[tokptr]:=
40960+trans[pp];tokptr:=tokptr+1;tokmem[tokptr]:=40960+trans[pp+1];
tokptr:=tokptr+1;tokmem[tokptr]:=141;tokptr:=tokptr+1;
begin red(pp,2,11,-2);{prod(24)};goto 31;end;
end else begin red(pp,0,1,-2);{prod(25)};goto 31;end{:161};
...
begin red(pp,4,11,-2);{prod(38)};goto 31;end;end;
end else if cat[pp+1]=11 then begin tokmem[tokptr]:=40960+trans[pp];
tokptr:=tokptr+1;tokmem[tokptr]:=140;tokptr:=tokptr+1;
tokmem[tokptr]:=40960+trans[pp+1];tokptr:=tokptr+1;
begin red(pp,2,16,-2);{prod(39)};goto 31;end;end{:164};
...
tokptr:=tokptr+1;begin red(pp,3,21,-2);{prod(40)};goto 31;end;
end else begin tokmem[tokptr]:=136;tokptr:=tokptr+1;
tokmem[tokptr]:=40960+trans[pp];tokptr:=tokptr+1;tokmem[tokptr]:=135;
tokptr:=tokptr+1;begin red(pp,1,17,0);{prod(41)};goto 31;end;end{:165};
9:{166:}begin red(pp,0,10,-3);{prod(42)};goto 31;end{:166};
11:{168:}if cat[pp+1]=11 then begin tokmem[tokptr]:=40960+trans[pp];
tokptr:=tokptr+1;tokmem[tokptr]:=140;tokptr:=tokptr+1;
tokmem[tokptr]:=40960+trans[pp+1];tokptr:=tokptr+1;
begin red(pp,2,11,-2);{prod(49)};goto 31;end;end{:168};
10:{169:}begin red(pp,0,11,-2);{prod(50)};goto 31;end{:169};
19:{170:}if cat[pp+1]=5 then begin red(pp,0,11,-2);{prod(51)};goto 31;
...
tokptr:=tokptr+1;tokmem[tokptr]:=36;tokptr:=tokptr+1;
tokmem[tokptr]:=40960+trans[pp+2];tokptr:=tokptr+1;
begin red(pp+1,2,3,+1);{prod(52)};goto 31;end;end;
end else if cat[pp+1]=1 then begin if cat[pp+2]=14 then begin sq(pp+1,2,
3,+1);{prod(53)};goto 31;end;
end else if cat[pp+1]=11 then begin tokmem[tokptr]:=40960+trans[pp];
tokptr:=tokptr+1;tokmem[tokptr]:=140;tokptr:=tokptr+1;
tokmem[tokptr]:=40960+trans[pp+1];tokptr:=tokptr+1;
begin red(pp,2,19,-2);{prod(54)};goto 31;end;end{:170};others:end;
pp:=pp+1;31:end{:149};end;30:{:175};
...
if ilk[p]=0 then{277:}begin q:=defval[p];
if q<>0 then if q<>10000000 then if not phasethree then begin repeat r:=
q;q:=xmem[q].dlinkfield;until(q=0)or(xmem[q].numfield>mm);
if xmem[r].numfield<>mm then begin qq:=0;q:=reflnk[0];
while refloc[q]>r do begin qq:=q;q:=reflnk[qq];end;
if refloc[q]=r then begin if safety=0 then if refsafety[q]=1 then
refsafety[q]:=0;
end else begin if newrefptr=maxnewrefs then begin writeln(stdout);
write(stderr,'! Sorry, ','new references',' capacity exceeded');error;
history:=3;jumpout;end;newrefptr:=newrefptr+1;reflnk[newrefptr]:=q;
reflnk[qq]:=newrefptr;refloc[newrefptr]:=r;
if xmem[defval[p]].dlinkfield=0 then refsafety[newrefptr]:=0 else
refsafety[newrefptr]:=safety;end;end;end;end{:277};
...
scrapbase:=scrapptr+1;safety:=1;Pascalparse;
...
scrapptr:=scrapbase-1;scrapbase:=savebase;safety:=0;Pascaltranslate:=p;
end;{:197}{198:}procedure outerparse;var bal:eightbits;p,q:textpointer;
...
b:eightbits;k,klimit:0..maxbytes;w:0..2;j:0..longbufsize;
...
if a=130 then if bytestart[curname+3]-bytestart[curname]=1 then begin if
...
if bytestart[curname+3]-bytestart[curname]=1 then begin if outptr=
linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=bytemem[curname mod 3,bytestart[curname]];
...
w:=curname mod 3;klimit:=bytestart[curname+3];curmodname:=curname;
...
buffer[limit]:=124;inmodulename:=true;outputPascal;inmodulename:=false;
loc:=saveloc;limit:=savelimit;end;end{:214};
begin if outptr=linelength then breakout;outptr:=outptr+1;
...
outptr:=outptr+1;outbuf[outptr]:=32;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=115;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=101;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=99;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=116;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=105;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=111;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=110;end;{237:}q:=curxref;
...
if(xmem[xmem[curxref].xlinkfield].numfield>flag)or(curxref<>xmem[q].
...
linelength then breakout;outptr:=outptr+1;outbuf[outptr]:=97;
if outptr=linelength then breakout;outptr:=outptr+1;outbuf[outptr]:=110;
if outptr=linelength then breakout;outptr:=outptr+1;outbuf[outptr]:=100;
if outptr=linelength then breakout;outptr:=outptr+1;outbuf[outptr]:=126;
end;end;30:{:237};begin if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=46;end;10:end;
...
{:260}{261:}{280:}procedure alphaout;label 10;var p,w,k:integer;
begin while true do begin if xx=xrefptr then goto 10;
if xmem[xx+1].numfield>mm then goto 10;xx:=xx+1;
if xmem[xx].numfield>10240 then if xmem[xx].dbackfield>0 then if ilk[
xmem[xx].dbackfield]=0 then begin flushbuffer(outptr,false,false);
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=92;if outptr=linelength then breakout;outptr:=outptr+1;
...
outbuf[outptr]:=105;end;p:=xmem[xx].dbackfield;w:=p mod 3;
for k:=bytestart[p]to bytestart[p+3]-1 do begin if outptr=linelength
then breakout;outptr:=outptr+1;outbuf[outptr]:=bytemem[w,k];end;
if xmem[xx].dtypefield>10 then xx:=xx+1;end;end;10:end;
{:280}{281:}procedure outconst(n:sixteenbits);var k:0..5;a:integer;
begin k:=0;a:=n;
if a>=32768 then begin begin if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=45;end;a:=65536-a;end;
repeat dig[k]:=a mod 10;a:=a div 10;k:=k+1;until a=0;repeat k:=k-1;
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=dig[k]+48;end;until k=0;end;
{:281}{282:}procedure outid(p:integer);
begin begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=92;end;
if ilk[p]=0 then if bytestart[p+3]-bytestart[p]=1 then begin if outptr=
linelength then breakout;outptr:=outptr+1;outbuf[outptr]:=124;
end else begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=92;end else begin if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=38;end;
if bytestart[p+3]-bytestart[p]=1 then begin if outptr=linelength then
breakout;outptr:=outptr+1;outbuf[outptr]:=bytemem[p mod 3,bytestart[p]];
end else outname(p);end;{:282}{283:}procedure betaout;label 30,888,31;
var k,p,q,w,xx,mmm:integer;begin p:=reflnk[0];
while p<>0 do begin flushbuffer(outptr,false,false);
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=92;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=91;end;xx:=refloc[p];q:=xmem[xx].dbackfield;w:=q mod 3;
for k:=bytestart[q]to bytestart[q+3]-1 do begin if outptr=linelength
then breakout;outptr:=outptr+1;outbuf[outptr]:=bytemem[w,k];end;
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=32;end;outconst(xmem[xx].numfield-10240);
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=32;end;{285:}mmm:=xmem[xx].numfield;
while true do begin if xmem[xx].dtypefield<>10 then if xmem[xx].numfield
=mmm then goto 30;if xx=xrefptr then begin xmem[xx].dtypefield:=0;
goto 30;end;
if xmem[xx+1].numfield>mmm then begin xmem[xx].dtypefield:=0;goto 30;
end;xx:=xx+1;end;30:{:285};
{284:}case xmem[xx].dtypefield of 0:begin if outptr=linelength then
breakout;outptr:=outptr+1;outbuf[outptr]:=92;
if outptr=linelength then breakout;outptr:=outptr+1;outbuf[outptr]:=110;
if outptr=linelength then breakout;outptr:=outptr+1;outbuf[outptr]:=111;
if outptr=linelength then breakout;outptr:=outptr+1;outbuf[outptr]:=110;
if outptr=linelength then breakout;outptr:=outptr+1;outbuf[outptr]:=101;
end;1:begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=61;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=109;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=97;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=99;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=114;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=111;end;
2:begin begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=61;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=36;end;outconst(xmem[xx].dnamefield);
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=36;end;end;3:begin if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=92;if outptr=linelength then breakout;
...
outptr:=outptr+1;outbuf[outptr]:=34;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=115;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=116;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=114;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=105;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=110;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=103;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=34;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=125;end;
4,8:begin begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=58;end;outid(xmem[xx].dnamefield);end;
5,9:begin begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=61;end;outid(xmem[xx].dnamefield);end;
6:begin begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=58;end;outid(packedname);
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=92;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=32;end;outid(xmem[xx].dnamefield);end;
7:begin begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=61;end;outid(packedname);
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=92;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=32;end;outid(xmem[xx].dnamefield);end;
11:begin begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=58;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=36;end;outconst(xmem[xx].dnamefield);goto 888;end;
12:begin begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=61;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=36;end;outconst(xmem[xx].dnamefield);goto 888;end;
13:begin begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=58;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=36;end;outid(xmem[xx].dnamefield);goto 888;end;
14:begin begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=61;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=36;end;outid(xmem[xx].dnamefield);goto 888;end;end;
goto 31;888:begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=92;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=116;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=111;end;
if xmem[xx+1].dtypefield=13 then outid(xmem[xx+1].dnamefield)else
outconst(xmem[xx+1].dnamefield);
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=36;end;31:{:284};
if refsafety[p]=1 then begin if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=32;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=37;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=63;end;p:=reflnk[p];end;end;
{:283}procedure PhaseI;begin{109:}phaseone:=true;phasethree:=false;
resetinput;modulecount:=0;changedmodule[0]:=false;skiplimbo;
changeexists:=false;
...
if nextcontrol=144 then begin nextcontrol:=getnext;
if nextcontrol=130 then begin lhs:=idlookup(0);nextcontrol:=getnext;
if(nextcontrol=30)or(nextcontrol=40)then begin deftype:=1;newxref(lhs);
end else if nextcontrol=61 then begin xrefswitch:=0;
nextcontrol:=getnext;defval[lhs]:=scanexp;defname:=defval[lhs];
deftype:=2;xrefswitch:=10240;
if abs(defname)>=32768 then deftype:=1 else if defname<0 then defname:=
defname+65536;newxref(lhs);end;end;
end else{116:}begin nextcontrol:=getnext;
...
var lhs:integer;begin{218:}resetinput;begin writeln(stdout);
write(stdout,'Writing the output file...');end;modulecount:=0;xx:=0;
copylimbo;finishline;flushbuffer(0,false,false);
{278:}for lhs:=1 to nameptr do if ilk[lhs]=0 then begin defval[lhs]:=0;
rhs:=xref[lhs];
while rhs<>0 do begin if xmem[rhs].numfield>10240 then begin xmem[rhs].
dlinkfield:=defval[lhs];defval[lhs]:=rhs;end;rhs:=xmem[rhs].xlinkfield;
end;end{:278};
while not inputhasended do{220:}begin modulecount:=modulecount+1;
mm:=modulecount+10240;reflnk[0]:=0;refloc[0]:=0;newrefptr:=0;safety:=0;
...
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=125;end;end{:224};
134,131,132,133,146,2:begin loc:=loc-2;nextcontrol:=getnext;
...
end else if nextcontrol=2 then begin if not phaseone then begin writeln(
stdout);write(stdout,'! Verbatim string should be in Pascal text only');
error;end;end;end;
...
outbuf[outptr]:=110;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=105;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=37;end;betaout;flushbuffer(outptr,false,false);
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=125;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=92;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=70;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=73;end;finishline;flushbuffer(0,false,false);{:238};
end{:220}{:218};end;begin initialize;
write(stdout,'This is TWILL, Version 4.5');
...
buffer[9]:=116;curname:=idlookup(7);constname:=curname;idfirst:=7;
buffer[7]:=100;buffer[8]:=105;buffer[9]:=118;curname:=idlookup(8);
idfirst:=8;buffer[8]:=100;buffer[9]:=111;curname:=idlookup(9);
idfirst:=4;buffer[4]:=100;buffer[5]:=111;buffer[6]:=119;buffer[7]:=110;
...
buffer[8]:=101;buffer[9]:=100;curname:=idlookup(13);packedname:=curname;
idfirst:=1;buffer[1]:=112;buffer[2]:=114;buffer[3]:=111;buffer[4]:=99;
...
while not changedmodule[kmodule]do kmodule:=kmodule+1;
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=92;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=99;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=104;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=32;end;outmod(kmodule);
repeat repeat kmodule:=kmodule+1 until changedmodule[kmodule];
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=44;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=32;end;outmod(kmodule);until kmodule=modulecount;
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=46;end;end{:241};end;finishline;
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=92;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=105;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=110;if outptr=linelength then breakout;outptr:=outptr+1;
...
if xref[curname]<>0 then begin c:=bytemem[curname mod 3,bytestart[
...
{253:}case ilk[curname]of 0:if bytestart[curname+3]-bytestart[curname]=1
then begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=92;if outptr=linelength then breakout;outptr:=outptr+1;
...
if curval<10240 then begin outmod(curval);
if curval+1=xmem[xmem[curxref].xlinkfield].numfield then begin begin if
outptr=linelength then breakout;outptr:=outptr+1;outbuf[outptr]:=45;
if outptr=linelength then breakout;outptr:=outptr+1;outbuf[outptr]:=45;
end;repeat curxref:=xmem[curxref].xlinkfield;curval:=curval+1;
until curval+1<>xmem[xmem[curxref].xlinkfield].numfield;outmod(curval);
end;end else begin begin if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=92;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=91;end;outmod(curval-10240);
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=93;end;end;curxref:=xmem[curxref].xlinkfield;
until curxref=0;begin if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=46;end;finishline{:254};
curname:=blink[curname];until curname=0;scrapptr:=scrapptr-1;
end{:252}else{251:}begin nextname:=trans[scrapptr];
repeat curname:=nextname;nextname:=blink[curname];
curbyte:=bytestart[curname]+curdepth;curbank:=curname mod 3;
if curbyte=bytestart[curname+3]then c:=0 else begin c:=bytemem[curbank,
