...
maxrow:integer;{:48}{55:}gfch:integer;gfchmod256:integer;
predpkloc:integer;maxn,minn:integer;maxm,minm:integer;rowptr:integer;
{:55}{78:}power:array[0..8]of integer;
{:78}{82:}comment:packed array[1..0]of 0..255;
{:82}{87:}checksum:integer;designsize:integer;hmag:integer;i:integer;
...
{:42}{49:}row:=xmallocarray(integer,MAXROW);maxrow:=MAXROW;
for i:=0 to 255 do status[i]:=0;{:49}{79:}power[0]:=1;
for i:=1 to 8 do power[i]:=power[i-1]+power[i-1];{:79}end;
...
{:43}{44:}procedure pkhalfword(a:integer);begin if a<0 then a:=a+65536;
...
{:46}{51:}{62:}procedure packandsendcharacter;var i,j,k:integer;
{65:}extra:integer;putptr:integer;repeatflag:integer;hbit:integer;
...
pkloc:=pkloc+1 end;end{:76}{:68};end{:62};procedure rowoverflow;
...
gfcom:integer;{58:}dotherows:boolean;on:boolean;state:boolean;
extra:integer;{:58}{61:}hppp,vppp:integer;q:integer;postloc:integer;
{:61}begin opengffile;
...
{53:}239,240,241,242:begin begin putbyte(gfcom+1,pkfile);
...
{57:}begin rowptr:=2;on:=false;extra:=0;state:=true;
