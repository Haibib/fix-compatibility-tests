...
#define maxkanji ( 3145727L ) 
#define maxkanjicode ( 3145727L ) 
...
initialize ( void ) 
{
  ...
  if ( verbose ) 
  {
    Fputs ( stderr ,  "This is upPLtoTF, Version 3.6-p240427" ) ;
    fprintf ( stderr , "%s\n",  versionstring ) ;
...
zprintjishex ( integer jiscode ) 
{
  byte dig[6]  ;
  byte i  ;
  dig [0 ]= ( jiscode / 65536L ) / 16 ;
  dig [1 ]= ( jiscode / 65536L ) % 16 ;
  dig [2 ]= ( jiscode / 4096 ) % 16 ;
  dig [3 ]= ( jiscode / 256 ) % 16 ;
  dig [4 ]= ( jiscode / 16 ) % 16 ;
  dig [5 ]= jiscode % 16 ;
  ...
  while ( i++ < for_end ) ;} 
  {register integer for_end; i = 2 ;for_end = 5 ; if ( i <= for_end) do 
    if ( dig [i ]< 10 ) 
    fprintf ( stderr , "%ld",  (long)dig [i ]) ;
    else switch ( dig [i ]) 
    {case 10 : 
...
zvalidjiscode ( integer cx ) 
{
  ...
  if ( ( cx > maxkanji ) || ( ! ischarkanji ( fromDVI ( cx ) ) ) || ( toDVI ( 
  fromDVI ( cx ) ) != cx ) ) 
  Result = false ;
  return Result ;
} 
...
getkanji ( void ) 
{
	...
	{
	  {
	    if ( charsonline > 0 ) 
	    { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
	    Fputs ( stderr ,  "This value shouldn't exceed jis code" ) ;
	    showerrorcontext () ;
	  } 
	  do {
	      getnext () ;
  ...
  else if ( ( ch == 'U' ) || ( ch == 'u' ) ) 
  {
    ...
    if ( ! validjiscode ( jiscode ) ) 
    {
      if ( charsonline > 0 ) 
      { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
      fprintf ( stderr , "%s%ld%s",  "ucs code " , (long)jiscode , " is invalid" ) ;
      showerrorcontext () ;
    } 
  } 
  ...
  {
    curchar = 32 ;
    jiscode = fromBUFF ( ustringcast ( buffer ) , loc + 4 , loc ) ;
    loc = loc + multistrlen ( ustringcast ( buffer ) , loc + 4 , loc ) - 1 ;
    if ( ( isinternalUPTEX () ) ) 
    {
      cx = fromBUFF ( ustringcast ( buffer ) , loc + 5 , loc + 1 ) ;
      if ( ( UVScombinecode ( jiscode , cx ) > 0 ) ) 
      {
	jiscode = UVScombinecode ( jiscode , cx ) ;
...
corrandcheck ( void ) 
{
  ...
  {register integer for_end; c = 0 ;for_end = 255 ; if ( c <= for_end) do 
	...
	do {
	    if ( hashinput ( ligptr , c ) ) 
	  {
		...
		{
		  charwd [g ]= sortin ( 1 , 0 ) ;
		  fprintf ( stderr , "%s%c",  "GLUE character examined by" , ' ' ) ;
		  printoctal ( c ) ;
		  fprintf ( stderr , "%s\n",  " had no CHARACTER spec." ) ;
		} 
	      } 
  ...
  if ( fileformat == 1 ) 
  {
    ...
    if ( ne > 0 ) 
    {register integer for_end; g = 0 ;for_end = ne - 1 ; if ( g <= for_end) 
    do 
      {
	{
	  c = exten [g ].b0 ;
	  ...
	  case 73 : 
		...
		nk = nk + 1 ;
		    ...
		    
		  {
		    if ( charsonline > 0 ) 
		    { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
		    fprintf ( stderr , "%s%ld%s",  "At most " , (long)maxkerns ,                     " different kerns are allowed" ) ;
		    showerrorcontext () ;
		  } 
		  krnptr = krnptr - 1 ;
		} 
		...
		if ( charsonline > 0 ) 
		{ putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
		fprintf ( stderr , "%s%ld%s",  "GLUEKERN table should never exceed " ,                 (long)maxligsteps , " GLUE/KRN commands" ) ;
		showerrorcontext () ;
	      } 
	      else nl = nl + 1 ;
	      lkstepended = true ;
	  ...
	  case 75 : 
		...
		ng = ng + 1 ;
		    ...
		    
		  {
		    if ( charsonline > 0 ) 
		    { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
		    fprintf ( stderr , "%s%ld%s",  "At most " , (long)maxkerns ,                     " different glues are allowed" ) ;
		    showerrorcontext () ;
		  } 
		  krnptr = krnptr - 1 ;
		} 
	      } 
	      ligkern [nl ].b2 = krnptr / 256 ;
	      ligkern [nl ].b3 = krnptr % 256 ;
	      if ( nl >= maxligsteps - 1 ) 
	      {
		if ( charsonline > 0 ) 
...
void mainbody( void ) {
  ...
  if ( lf < 0 ) 
  {
    fprintf ( stderr , "%s\n",  "The total number of words in the TFM file too large!"     ) ;
    uexit ( 1 ) ;
  } 
  if ( fileformat == 3 ) 
  {
  ...
  if ( fileformat != 1 ) 
  {
	...
	if ( kanjitype [kanjitypeindex ]> 0 ) 
	{
	  putbyte ( ( ( indextojis ( kanjitypeindex ) ) % 65536L ) / 256 , 
	  tfmfile ) ;
	  putbyte ( ( ( indextojis ( kanjitypeindex ) ) % 65536L ) % 256 , 
	  tfmfile ) ;
	  putbyte ( ( indextojis ( kanjitypeindex ) ) / 65536L , tfmfile ) ;
