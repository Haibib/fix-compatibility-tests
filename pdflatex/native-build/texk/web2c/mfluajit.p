...
endif('TEXMF_DEBUG'){:9}program MF;label{6:}1,9999;
{:6}const{11:}maxinternal=300;stacksize=300;maxstrings=1048574;
stringvacancies=74000;poolsize=10000000;movesize=20000;maxwiggle=1000;
...
eightbitp:cinttype;haltonerrorp:cinttype;haltingonerrorp:boolean;
quotedfilename:boolean;{:13}{20:}xord:array[ASCIIcode]of ASCIIcode;
...
stopatspace:=true;haltingonerrorp:=false;{:1217}end;
{57:}procedure println;begin case selector of 3:begin writeln(stdout);
...
goto 10;clearforerrorprompt;begin;print(265);terminput;end;
...
endif('TEXMF_DEBUG')69:if fileptr>0 then if inputstack[fileptr].
namefield>=256 then begin editnamestart:=strstart[inputstack[fileptr].
namefield];
...
internal[17]:=systime*65536;internal[16]:=sysday*65536;
internal[15]:=sysmonth*65536;internal[14]:=sysyear*65536;end;
...
strref[luacurexp])else flushstring(luacurexp);end;end;
begin if strref[luacurexp]<127 then if strref[luacurexp]>1 then decr(
strref[luacurexp])else flushstring(luacurexp);end;flushcurexp(0);
...
{790:}begin write(logfile,'This is MFLuaJIT, Version 2.71828182',
'-1.0.0-alpha');slowprint(baseident);print(756);printint(sysday);
...
printchar(58);printdd(systime mod 60);
if translatefilename then begin writeln(logfile);write(logfile,'(');
fputs(translatefilename,logfile);write(logfile,')');end;end{:790};
...
mfluaPREmakespecrhs(rhs);
rhs:=makespec(rhs,mem[curpen+9].int,internal[5]);
mfluaPOSTmakespecrhs(rhs);
...
mfluaPREmakespeclhs(lhs);
lhs:=makespec(lhs,mem[curpen+9].int,internal[5]);
mfluaPOSTmakespeclhs(lhs);revturns:=false;
...
dumpthings(baseengine[0],x);libcfree(baseengine);dumpint(258590632);
...
libcfree(baseengine);undumpint(x);if x<>258590632 then begin;
writeln(stdout,'---! ',stringcast(nameoffile+1),
' made by different executable version');goto 6666;end;
...
{'BREAKPOINT'}end else begin read(stdin,n);
case m of{1213:}1:printword(mem[n]);2:printint(mem[n].hh.lh);
3:printint(mem[n].hh.rh);4:begin printint(eqtb[n].lh);printchar(58);
printint(eqtb[n].rh);end;5:printvariablename(n);6:printint(internal[n]);
7:doshowdependencies;9:showtokenlist(n,0,100000,0);10:slowprint(n);
11:checkmem(n>0);12:searchmem(n);13:begin read(stdin,l);
printcmdmod(n,l);end;14:for k:=0 to n do print(buffer[k]);
15:panicking:=not panicking;{:1213}others:print(63)end;end;end;10:end;
...
{:55}{61:}write(stdout,'This is MFLuaJIT, Version 2.71828182',
'-1.0.0-alpha');write(stdout,versionstring);
...
fixdateandtime;initrandoms(systime+sysday*65536);
{70:}if interaction=0 then selector:=0 else selector:=1{:70};
