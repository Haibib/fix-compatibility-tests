...
error ( void ) 
{
  ...
  while ( true ) {
	...
	if ( inputstack [fileptr ].namefield >= 256 ) 
	{
...
fixdateandtime ( void ) 
{
  ...
  internal [17 ]= systime * 65536L ;
  internal [16 ]= sysday * 65536L ;
  internal [15 ]= sysmonth * 65536L ;
  internal [14 ]= sysyear * 65536L ;
} 
halfword 
...
zfillenvelope ( halfword spechead ) 
{
  ...
  do {
    ...
    if ( odd ( octantnumber [octant ]) ) 
    {
      ...
      xycorr [octant ]) ;
      {register integer for_end; n = 0 ;for_end = n1 - n0 - 1 ; if ( n <= 
      for_end) do 
	envmove [n ]= mm0 ;
      while ( n++ < for_end ) ;} 
      envmove [n1 - n0 ]= mm1 ;
...
zmakeellipse ( scaled majoraxis , scaled minoraxis , angle theta ) 
{
  ...
  if ( ( majoraxis == minoraxis ) || ( theta % 94371840L == 0 ) ) 
  {
    ...
    alpha = takefraction ( takefraction ( majoraxis , makefraction ( gamma , 
    beta ) ) , ncos ) - takefraction ( takefraction ( minoraxis , makefraction 
    ( delta , beta ) ) , nsin ) ;
    alpha = ( alpha + 32768L ) / 65536L ;
    gamma = pythadd ( takefraction ( majoraxis , ncos ) , takefraction ( 
    minoraxis , nsin ) ) ;
  } 
...
openlogfile ( void ) 
{
  ...
  logopened = true ;
  {
    fprintf ( logfile , "%s%s",  "This is MFLuaJIT, Version 2.71828182" , "-1.0.0-alpha"     ) ;
    slowprint ( baseident ) ;
    print ( 756 ) ;
    printint ( sysday ) ;
    printchar ( 32 ) ;
