...
boolean fntofmp[maxfonts + 1]  ;
integer fntofmlv[maxfonts + 1]  ;
...
initialize ( void ) 
{
  ...
  initkanji () ;
  parsearguments () ;
  Fputs ( stdout ,  "This is upDVItype, Version 3.6-p240427" ) ;
  Fputs ( stdout ,  " (" ) ;
...
zinTFM ( integer z ) 
{
  ...
  else if ( ( lh == 0 ) ) 
  {
    ...
    fntofmp [nf ]= true ;
    curofmlv = b2 * 256 + b3 ;
    fntofmlv [nf ]= curofmlv ;
    Fputs ( stdout ,  " (OFM level" ) ;
    fprintf ( stdout , "%ld",  (long)( fntofmlv [nf ]) ) ;
    putc ( ')' ,  stdout );
  } 
  else {
  ...
  {register integer for_end; k = 1 ;for_end = kk ; if ( k <= for_end) do 
    {
      if ( eof ( tfmfile ) ) 
      goto lab9997 ;
      readtfmword () ;
  ...
  {register integer for_end; k = 1 ;for_end = nt ; if ( k <= for_end) do 
    {
      readtfmword () ;
      jfmcharcode [jfmchartypecount ]= b0 * 256 + b1 + b2 * 65536L ;
      jfmchartype [jfmchartypecount ]= b3 ;
...
zoutkanji ( integer c ) 
{
  ...
  if ( ( isinternalUPTEX () ) ) 
  {
	...
	{
	  textptr = textptr + 1 ;
	  textbuf [textptr ]= BYTE1 ( v ) ;
	} 
	if ( BYTE2 ( v ) != 0 ) 
	{
	  textptr = textptr + 1 ;
	  textbuf [textptr ]= BYTE2 ( v ) ;
	} 
	if ( BYTE3 ( v ) != 0 ) 
	{
	  textptr = textptr + 1 ;
	  textbuf [textptr ]= BYTE3 ( v ) ;
	} 
	textptr = textptr + 1 ;
	textbuf [textptr ]= BYTE4 ( v ) ;
      } 
      jj = jj + 1 ;
    } 
  } 
...
dopage ( void ) 
{
  ...
  while ( true ) {
	...
	else if ( ( fntofmp [curfont ]== true ) && ( isinternalUPTEX () ) ) 
	{
	  outkanji ( p ) ;
	  if ( o < 133 ) 
	  {
	...
	if ( ! ptexp && ( outmode == 4 ) ) 
	{
	  fprintf ( stderr , "%s%s%c\n",  "Bad DVI file: " ,           "dir command within normal dvi file" , '!' ) ;
	  uexit ( 1 ) ;
	} 
	if ( outmode > 0 ) 
	{
    ...
    vv = vv + pixelwidth [widthbase [curfont ]+ p ];
...
scanbop ( void ) 
{
  ...
  do {
	...
	if ( outmode > 0 ) 
	{
	  flushtext () ;
	  showing = true ;
	  fprintf ( stdout , "%ld%s%s%ld%c%ld",  (long)a , ": " , "fntdef" , (long)k - 242 , ' ' , (long)p ) ;
	  if ( showopcodes && ( o >= 128 ) ) 
	  fprintf ( stdout , "%s%ld%c",  " {" , (long)o , '}' ) ;
	} 
      } 
