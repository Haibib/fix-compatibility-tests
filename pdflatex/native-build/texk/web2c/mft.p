...
{:55}{72:}translation:array[ASCIIcode]of namepointer;i:ASCIIcode;
{:72}{74:}trle,trge,trne,tramp,trsharp,trskip,trps,trquad:namepointer;
{:74}{75:}curtype:eightbits;curtok:integer;prevtype:eightbits;
prevtok:integer;scannerstatus:0..2;{:75}{77:}startofline:boolean;
emptybuffer:boolean;{:77}{78:}charclass:array[ASCIIcode]of 0..21;
{:78}{86:}outbuf:array[0..linelength]of ASCIIcode;outptr:0..linelength;
outline:integer;{:86}{114:}changename,texname:constcstring;
stylename:array[0..31]of constcstring;nstylename:cinttype;
istylename:cinttype;metapost:cinttype;{:114}{29:}procedure error;
var k,l:0..bufsize;
...
begin{113:}case history of 0:begin writeln(stdout);
...
end;{:31}{115:}procedure parsearguments;const noptions=5;
var longoptions:array[0..noptions]of getoptstruct;
getoptreturnval:integer;optionindex:cinttype;currentoption:0..noptions;
suffix:constcstring;begin{121:}metapost:=false;{:121};
{116:}currentoption:=0;longoptions[currentoption].name:='help';
...
{:116}{117:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:117}{118:}longoptions[currentoption].name:='change';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:118}{119:}longoptions[currentoption].name:='style';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:119}{120:}longoptions[currentoption].name:='metapost';
...
{:120}{122:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:122};nstylename:=0;
...
{:115}procedure initialize;var{14:}i:0..255;{:14}{56:}h:0..hashsize;
...
{:54}{57:}for h:=0 to hashsize-1 do hash[h]:=0;{:57}{76:}curtype:=1;
curtok:=0;{:76}{79:}for i:=48 to 57 do charclass[i]:=0;charclass[46]:=1;
...
end else write(texfile,'\input mftmac');{:88}{90:}outbuf[0]:=92;{:90};
...
if not inputln(mffile)then inputhasended:=true else if changelimit>0
then checkchange;end{:46};if changing then goto 20;end;end;end;
...
4:{82:}while true do begin if buffer[loc]=34 then begin loc:=loc+1;
begin curtype:=7;curtok:=idfirst;goto 10;end;end;
if loc=limit then{83:}begin begin writeln(stdout);
write(stdout,'! Incomplete string will be ignored');error;end;goto 25;
end{:83};loc:=loc+1;end{:82};5,6,7,8:goto 31;
20:{84:}begin begin writeln(stdout);
...
{:80}{87:}procedure flushbuffer(b:eightbits;percent:boolean);label 30;
...
end{:92};d:=outbuf[k];if d=32 then begin flushbuffer(k,false);goto 10;
end;if(d=92)and(outbuf[k-1]<>92)then begin flushbuffer(k-1,true);
goto 10;end;k:=k-1;end;10:end;{:91}{93:}procedure outstr(p:namepointer);
var k:0..maxbytes;
...
end else if curtype=5 then goto 20{:98};
21:case curtype of 6:{105:}if buffer[loc]=47 then if charclass[buffer[
...
outbuf[outptr]:=55;end;copy(curtok);end{:99};
33,32:{125:}begin begin if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=92;if outptr=linelength then breakout;
...
outptr:=outptr+1;outbuf[outptr]:=36;end;scannerstatus:=0;end{:125};
0:outstr(trquad);
1,5:{110:}begin begin if outptr=linelength then breakout;
...
{:100}{101:}16,27,12:outname(curtok);
...
{:102}{103:}31:begin outmacandname(50,curtok);
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=92;if outptr=linelength then breakout;outptr:=outptr+1;
...
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=125;end{:104};end;
{:103}28,29:{108:}begin if curtype=28 then begin if outptr=linelength
...
outbuf[outptr]:=114;end;goto 20;end;end{:108};
3:{109:}begin idfirst:=loc;loc:=limit;copy(idfirst);
...
end;goto 21;end;emptybuffer:=true;goto 20;end{:111};
30,35,36:{106:}begin if bytestart[curtok+1]-bytestart[curtok]=1 then
...
21;
if(curtype=6)or(curtype>=30)then{107:}begin begin if outptr=linelength
...
end{:107}else if curtype=27 then outstr(trsharp)else goto 21;end;end;
end{:106};end;end;10:end;{:97}{112:}begin initialize;
write(stdout,'This is MFT, Version 2.1');writeln(stdout,versionstring);
...
buffer[17]:=101;curtok:=lookup;ilk[curtok]:=(30);idfirst:=3;
buffer[3]:=116;buffer[4]:=114;buffer[5]:=97;buffer[6]:=99;
buffer[7]:=105;buffer[8]:=110;buffer[9]:=103;buffer[10]:=114;
buffer[11]:=101;buffer[12]:=115;buffer[13]:=116;buffer[14]:=111;
buffer[15]:=114;buffer[16]:=101;buffer[17]:=115;curtok:=lookup;
ilk[curtok]:=(30);{:68}{69:}idfirst:=14;buffer[14]:=121;buffer[15]:=101;
buffer[16]:=97;buffer[17]:=114;curtok:=lookup;ilk[curtok]:=(30);
idfirst:=13;buffer[13]:=109;buffer[14]:=111;buffer[15]:=110;
buffer[16]:=116;buffer[17]:=104;curtok:=lookup;ilk[curtok]:=(30);
idfirst:=15;buffer[15]:=100;buffer[16]:=97;buffer[17]:=121;
curtok:=lookup;ilk[curtok]:=(30);idfirst:=14;buffer[14]:=116;
buffer[15]:=105;buffer[16]:=109;buffer[17]:=101;curtok:=lookup;
ilk[curtok]:=(30);idfirst:=10;buffer[10]:=99;buffer[11]:=104;
buffer[12]:=97;buffer[13]:=114;buffer[14]:=99;buffer[15]:=111;
buffer[16]:=100;buffer[17]:=101;curtok:=lookup;ilk[curtok]:=(30);
idfirst:=11;buffer[11]:=99;buffer[12]:=104;buffer[13]:=97;
buffer[14]:=114;buffer[15]:=101;buffer[16]:=120;buffer[17]:=116;
...
{:71}{123:}if not metapost then begin idfirst:=6;buffer[6]:=97;
...
{:123}{124:}if metapost then begin idfirst:=9;buffer[9]:=97;
...
ilk[curtok]:=(9);end;{:124};{73:}for i:=0 to 255 do translation[i]:=0;
...
nameptr:=nameptr+1;bytestart[nameptr]:=byteptr;trquad:=curtok;{:73};
...
{49:}if changelimit<>0 then begin for loc:=0 to changelimit-1 do buffer[
