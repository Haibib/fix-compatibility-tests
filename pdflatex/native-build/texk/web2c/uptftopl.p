...
{:109}{113:}tfmname,plname:constcstring;{:113}{114:}thiscode:integer;
thistype:integer;typeindex:integer;typenum:integer;typecount:integer;
kanjiindex:integer;kanjitype:array[0..3145727]of-1..255;
{:114}{117:}xchr:array[char]of byte;
{:117}{100:}procedure parsearguments;const noptions=6;
...
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:106}{111:}longoptions[currentoption].name:='kanji';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:111}{112:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:112};
...
usagehelp(UPTFTOPLHELP,'issue@texjp.org');
end else if(strcmp(longoptions[optionindex].name,'version')=0)then begin
printversionandexit('This is upTFtoPL, Version 3.3-p240427',nil,
'D.E. Knuth',nil);
...
'This is upTFtoPL, Version 3.3-p240427');writeln(stderr,versionstring);
...
{:64}{118:}for k:=0 to 31 do xchr[k]:='?';
for k:=32 to 255 do xchr[k]:=k;{:118}end;
...
writeln(stderr,'Sorry, but I can''t go on; are you sure this is a TFM?')
;goto 9999;end;if tfm[4]>127 then begin writeln(stderr,
...
end{:76}else if fileformat<>1 then{115:}begin if((tfm[k+1]<bc)or(tfm[k+1
]>ec)or(tfm[4*(charbase+tfm[k+1])]=0))then if tfm[k+1]<>boundarychar
then begin perfect:=false;if charsonline>0 then writeln(stderr,' ');
charsonline:=0;
write(stderr,'Bad TFM file: ','Glue step for',' nonexistent character ')
;printoctal(tfm[k+1]);writeln(stderr,'.');tfm[k+1]:=bc;end;left;
write(plfile,'GLUE');outchar(tfm[k+1]);r:=3*(256*tfm[k+2]+tfm[k+3]);
if r>=ng then begin begin perfect:=false;
if charsonline>0 then writeln(stderr,' ');charsonline:=0;
writeln(stderr,'Bad TFM file: ','Glue index too large.');end;
write(plfile,' R 0.0 R 0.0 R 0.0');
end else begin outfix(4*(gluebase+r));outfix(4*(gluebase+r+1));
outfix(4*(gluebase+r+2));end;right;
end{:115}else{77:}begin if((tfm[k+1]<bc)or(tfm[k+1]>ec)or(tfm[4*(
...
end{:78};end;{:98}{99:}{119:}procedure outkanji(jiscode:integer);
var cx,v,nn,jj:integer;i:0..5;
begin if(charcodeformat=charcodeoctal)or(jiscode<128)then begin cx:=
jiscode;
...
write(plfile,dig[i])else case dig[i]of 10:write(plfile,'A');
11:write(plfile,'B');12:write(plfile,'C');13:write(plfile,'D');
14:write(plfile,'E');15:write(plfile,'F');end;end;
for i:=2 to 5 do if dig[i]<10 then write(plfile,dig[i])else case dig[i]
of 10:write(plfile,'A');11:write(plfile,'B');12:write(plfile,'C');
13:write(plfile,'D');14:write(plfile,'E');15:write(plfile,'F');end;
end else begin if(isinternalUPTEX)then begin cx:=fromDVI(jiscode);
nn:=UVSgetcodepointlength(cx);jj:=1;
while jj<=nn do begin v:=UVSgetcodepointinsequence(cx,jj);
if(v>0)then begin v:=UCStoUTF8(v);
if BYTE1(v)<>0 then write(plfile,xchr[BYTE1(v)]);
if BYTE2(v)<>0 then write(plfile,xchr[BYTE2(v)]);
if BYTE3(v)<>0 then write(plfile,xchr[BYTE3(v)]);
write(plfile,xchr[BYTE4(v)]);end;jj:=jj+1;
end end else begin cx:=toBUFF(fromDVI(jiscode));
...
write(plfile,xchr[BYTE4(cx)]);end;end;end;
{:119}{120:}function validjiscode(cx:integer):boolean;
begin validjiscode:=true;
if(cx>3145727)or(not ischarkanji(fromDVI(cx)))or(toDVI(fromDVI(cx))<>cx)
then validjiscode:=false;end;
{:120}{121:}function indextojis(ix:integer):integer;
begin indextojis:=ix;end;
{:121}{122:}function jistoindex(cx:integer):integer;
begin jistoindex:=cx;end{:122};begin initialize;
...
if level=2 then right{:71};right;
if fileformat=1 then begin{90:}hashptr:=0;yligcycle:=256;
...
if fileformat<>1 then begin{116:}thiscode:=tfm[4*(typebase+0)+0]*256+tfm
[4*(typebase+0)+1]+tfm[4*(typebase+0)+2]*256*256;
thistype:=tfm[4*(typebase+0)+3];
...
end;for kanjiindex:=0 to 3145727 do kanjitype[kanjiindex]:=-1;
for typeindex:=1 to nt-1 do begin thiscode:=tfm[4*(typebase+typeindex)+0
]*256+tfm[4*(typebase+typeindex)+1]+tfm[4*(typebase+typeindex)+2]*256
*256;thistype:=tfm[4*(typebase+typeindex)+3];
...
for kanjiindex:=0 to 3145727 do if kanjitype[kanjiindex]=typenum then
...
' has no characters in it!');end;outln;right;end;{:116};
